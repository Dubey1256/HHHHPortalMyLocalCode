                ...
                <div className="tbl-headings justify-content-between" style={{ background: 'white', borderBottom: "none" }}>
                    <div className="">
                        ...
                        {items?.showEmailIcon === true ? <>
                            <div onClick={() => items?.openCreationAllStructure("sendEmail")}>
                            <div className="col-sm-12 p-0">
                            <div  className="alignCenter mb-1"> <a className="teamIcon p-0 me-1"><span title="send email" style={{ color: `${portfolioColor}`, backgroundColor: `${portfolioColor}` }} className="svg__iconbox svg__icon--mail"></span></a>
                            <a>Send Email</a></div>
                                </div>   
                            </div>
                        </> : ''}
                        ...
                        {items?.hideOpenNewTableIcon != true ? <>
                            ...
                            <div onClick={() => items?.openTaskAndPortfolioMulti()}> 
                             <div  className="alignCenter mb-1 col-sm-12 p-0"><a  title='Open in New Tab' className="openWebIcon p-0 me-1"><span style={{ color: `${portfolioColor}`, backgroundColor: `${portfolioColor}` }} className="svg__iconbox svg__icon--openWeb"></span></a>
                             <a>Open in New Tab</a></div>   
                            </div>
                            ...
                            <div onClick={() => items?.exportToExcel()}>
                            <div  className="alignCenter mb-1 col-sm-12"><a className='excal me-1' title='Export to Excel' ><RiFileExcel2Fill style={{ color: `${portfolioColor}` }} /></a><a>Export to Excel</a></div>
                            </div>


                        {items?.SmartTimeIconShow === true && items?.AllListId?.isShowTimeEntry === true && 
                        <div><div className="alignCenter mb-1 col-sm-12"><a className='smartTotalTime me-1' title="Load SmartTime of AWT" onClick={() => items?.openCreationAllStructure("Smart-Time")} > <BsClockHistory style={{ color: `${portfolioColor}` }} /></a>Load SmartTime of AWT</div></div>
                        }
                        ...
                        <div onClick={() => { items?.setGlobalFilter(''); items?.setColumnFilters([]); items?.setRowSelection({}); }} className="alignCenter mb-1 col-sm-12"><a className='brush me-1'><i className="fa fa-paint-brush hreflink" style={{ color: `${portfolioColor}` }} aria-hidden="true" title="Clear All" ></i></a> <a>Clear All</a></div>

                           <div onClick={() => items?.downloadPdf()}>
                            {/* <a>Print</a> */}
                           <div  className="alignCenter mb-1 col-sm-12"><a className='Prints me-1' >
