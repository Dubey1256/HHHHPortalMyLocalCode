  ...
  return (
    <>
      <div className="container-2 mx-1">
        <span className="icon">
          <BsSearch style={{ color: `${portfolioColor}` }} />
        </span>
        <input
          type="search"
          id="search"
...
export function Filter({
  ...
  return (
    <input
      style={{ width: "100%" }}
      className="me-1 my-1 mx-1 on-search-cross"
      // type="text"
  ...
  return ({ table }: any) => (
    <>
      {hasExpanded && isHeaderNotAvlable != true && (
        <>
          <span
            className="border-0 bg-Ff ms-1"
            {...{ onClick: table.getToggleAllRowsExpandedHandler() }}
          >
            {table.getIsAllRowsExpanded() ? (
              <SlArrowDown
                style={{ color: portfolioColor, width: "12px" }}
                title="Tap to collapse the childs"
              />
            ) : (
              <SlArrowRight
                style={{ color: portfolioColor, width: "12px" }}
      ...
      {hasCheckbox && (
        <span
          style={hasExpanded ? { marginLeft: "7px", marginBottom: "5px" } : {}}
        >
  ...
  return ({ row, getValue, table }: any) => (
    <div className="alignCenter">
      {hasExpanded && row.getCanExpand() && (
        <div
          className="border-0 alignCenter"
          {...{
        ...
        >
          {row.getIsExpanded() ? (
            <SlArrowDown
              title={"collapse " + `${row.original.Title}` + " childs"}
              style={{
            ...
            <SlArrowRight
              title={"Expand" + `${row.original.Title}` + "childs"}
              style={{
                color: `${row?.original?.PortfolioType?.Color}`,
                width: "12px"
              }}
          ...
          row.original.subRows?.length ? (
                ...
                onClick: () => {
                  if (!forceExpanded.includes(row.id)) {
                    ...
                    const temp = Object.keys(coreIds).filter(
                      (item: any) =>
                        item === row.id || item.startsWith(row.id + ".")
                    );
            ...
            >
              ...
              (row.getCanExpand() &&
                ...
                <FaMinus
                  style={{ color: `${row?.original?.PortfolioType?.Color}` }}
                />
              ) : (
                <FaPlus
                  style={{
...
const GlobalCommanTable = (items: any, ref: any) => {
  ...
  const [showTeamMemberOnCheck, setShowTeamMemberOnCheck] =
    React.useState(false);
  const [globalSearchType, setGlobalSearchType] = React.useState("ALL");
  const [selectedFilterPanelIsOpen, setSelectedFilterPanelIsOpen] =
    React.useState(false);
  ...
  const [selectedFilterPannelData, setSelectedFilterPannelData] =
    React.useState({
      Title: { Title: "Title", Selected: true },
      commentsSearch: { commentsSearch: "commentsSearch", Selected: true },
      descriptionsSearch: {
        descriptionsSearch: "descriptionsSearch",
        Selected: true
      }
    });
  ...
  const customGlobalSearch = (row: any, id: any, query: any) => {
    ...
    if (
      (selectedFilterPannelData?.Title?.Title === id &&
        selectedFilterPannelData?.Title?.Selected === true) ||
      (selectedFilterPannelData?.commentsSearch?.commentsSearch === id &&
        selectedFilterPannelData?.commentsSearch?.Selected === true) ||
      (selectedFilterPannelData?.descriptionsSearch?.descriptionsSearch ===
        id &&
        selectedFilterPannelData?.descriptionsSearch?.Selected === true)
    ) {
  ...
  const table: any = useReactTable({
    ...
    enableMultiRowSelection:
      items?.multiSelect === false ? items?.multiSelect : true,
    ...
    getPaginationRowModel:
      showPagination === true ? getPaginationRowModel() : null,
  ...
  React.useEffect(() => {
    if (columnFilters.length > 0 || globalFilter.length > 0) {
      if (AfterSearch != undefined && AfterSearch.length > 0) {
        AfterSearch?.map((Comp: any) => {
          if (
            ...
            Comp.columnFilters.PortfolioStructureID == true ||
      ...
      let ShowingData = {
        ...
        SubComponentCopy: SubComponentCopy,
        FeatureCopy: FeatureCopy,
        FilterShowhideShwingData: FilterShowhideShwingData
      };
  ...
  React.useEffect(() => {
    if (AfterSearch != undefined && AfterSearch.length > 0) {
      portfolioTypeData?.filter((count: any) => {
        count[count.Title + "numberCopy"] = 0;
      });
      items?.taskTypeDataItem?.filter((taskLevelcount: any) => {
        taskLevelcount[taskLevelcount.Title + "numberCopy"] = 0;
      });
      AfterSearch?.map((Comp: any) => {
        if (columnFilters.length > 0 || globalFilter.length > 0) {
  ...
  const openTaskAndPortfolioMulti = () => {
    table?.getSelectedRowModel()?.flatRows?.map((item: any) => {
      ...
      if (item?.original?.siteType === "Master Tasks") {
        window.open(
          `${siteUrl}/SitePages/Portfolio-Profile.aspx?taskId=${item?.original?.Id}`,
          "_blank"
        );
      } else if (item?.original?.siteType === "Project") {
        window.open(
          `${siteUrl}/SitePages/Project-Management.aspx?taskId=${item?.original?.Id}`,
          "_blank"
        );
      } else {
        window.open(
          `${siteUrl}/SitePages/Task-Profile.aspx?taskId=${item?.original?.Id}&Site=${item?.original?.siteType}`,
          "_blank"
        );
      }
    });
    ...
    if (expendedTrue != true) {
      if (
        ...
        const allKeys = Object.keys(
          table.getFilteredRowModel().rowsById
  ...
  return (
    <>
      {showHeader === true && (
        <div
          className="tbl-headings justify-content-between mb-1 fixed-Header top-0"
          style={{ background: "#e9e9e9" }}
        >
          <span className="leftsec">
            {showingAllPortFolioCount === true ? (
              <div className="mb-1">
                <label style={{ color: `${portfolioColor}` }}>Showing</label>
                    ...
                    <>
                      {isShowingDataAll === true ? (
                        <>
                          ...
                          >
                            {` ${type[type.Title + "numberCopy"]} `} of{" "}
                          </label>{" "}
                          <label
                            style={{ color: `${portfolioColor}` }}
                            className="ms-0"
                          >{` ${type[type.Title + "number"]} `}</label>
                          <label
                            style={{ color: `${portfolioColor}` }}
                            className="ms-1"
                          >
                        ...
                        <>
                          <label
                            ...
                            style={{ color: `${portfolioColor}` }}
                          >
                            {` ${type[type.Title + "filterNumber"]} `} of{" "}
                          </label>{" "}
                          <label
                            style={{ color: `${portfolioColor}` }}
                            className="ms-0"
                        ...
                        <>
                          {isShowingDataAll === true ? (
                            <>
                              ...
                              {index < items?.taskTypeDataItem?.length - 1 && (
                                <label
                                  style={{ color: `${portfolioColor}` }}
                                  className="ms-1"
                                >
              ...
              <span
                style={{ color: `${portfolioColor}` }}
                className="Header-Showing-Items"
            ...
            />
            <span
              className="svg__iconbox svg__icon--setting"
              style={{ backgroundColor: `${portfolioColor}` }}
              onClick={() => setSelectedFilterPanelIsOpen(true)}
            ></span>
            <span className="ms-1">
              <select
                style={{ height: "30px", color: `${portfolioColor}` }}
                className="w80"
          ...
          <span className="toolbox">
                ...
                <>
                  {items?.PortfolioFeature === "Feature" ? (
                    ...
                    >
                      ...
                      table?.getSelectedRowModel()?.flatRows[0]?.original
                        ?.SharewebTaskType?.Title != "Activities" &&
                      table?.getSelectedRowModel()?.flatRows[0]?.original
                        ?.SharewebTaskType?.Title != "Workstream" &&
                      table?.getSelectedRowModel()?.flatRows[0]?.original
                        ?.SharewebTaskType?.Title != "Task") ||
                    ...
                    <button
                      ...
                      title=" Add Structure"
                      onClick={() => openCreationAllStructure("Add Structure")}
                    >
                      {" "}
                      Add Structure{" "}
                    </button>
                  ...
                  {items?.protfolioProfileButton != true && (
                    <>
                      {table?.getSelectedRowModel()?.flatRows.length === 1 ? (
                        <button
                          type="button"
                          ...
                          onClick={() =>
                            openCreationAllStructure("Add Activity-Task")
                          }
                        >
                          Add Activity-Task
                        </button>
                ...
                <>
                  {table?.getSelectedRowModel()?.flatRows.length < 2 ? (
                    <button
                      ...
                      onClick={() =>
                        openCreationAllStructure("Add Workstream-Task")
                      }
                    >
                      Add Workstream-Task
                    </button>
            ...
            {table?.getSelectedRowModel()?.flatRows?.length > 0 ? (
              <a className="teamIcon" onClick={() => ShowTeamFunc()}>
                <span
                  ...
                  className="svg__iconbox svg__icon--team"
                ></span>
              </a>
            ) : (
              <a className="teamIcon">
                <span
                  title="Create Teams Group"
            ...
            {table?.getSelectedRowModel()?.flatRows?.length > 0 ? (
              <a
                onClick={() => openTaskAndPortfolioMulti()}
                title="Open in new tab"
                className="openWebIcon p-0"
              >
                <span
                  style={{
                    color: `${portfolioColor}`,
                  ...
                  className="svg__iconbox svg__icon--openWeb"
                ></span>
              </a>
            ) : (
              <a className="openWebIcon p-0" title="Open in new tab">
                <span
                  className="svg__iconbox svg__icon--openWeb"
                  style={{ backgroundColor: "gray" }}
                ></span>
            ...
            <a
              className="excal"
              title="Export to excal"
              onClick={() => exportToExcel()}
            >
              <RiFileExcel2Fill style={{ color: `${portfolioColor}` }} />
            </a>

            {items?.SmartTimeIconShow === true &&
              items?.AllListId?.isShowTimeEntry === true && (
                <a
                  className="smartTotalTime"
                  title="Load SmartTime of AWT"
                  onClick={() => openCreationAllStructure("Smart-Time")}
                >
                  {" "}
            ...
            <a className="brush">
              <i
                className="fa fa-paint-brush hreflink"
                style={{ color: `${portfolioColor}` }}
                aria-hidden="true"
                title="Clear All"
                onClick={() => {
      ...
      <table
        className="SortingTable table table-hover mb-0"
        id="my-table"
        style={{ width: "100%" }}
      >
        <thead
            ...
            return (
              <tr className={row?.original?.lableColor} key={row.id}>
                {row.getVisibleCells().map((cell: any) => {
                  return (
                    <td
                      className={row?.original?.boldRow}
                      key={cell.id}
                      style={
      ...
      {ShowTeamPopup === true && items?.TaskUsers?.length > 0 ? (
        <ShowTeamMembers
          props={table?.getSelectedRowModel()?.flatRows}
          callBack={showTaskTeamCAllBack}
          TaskUsers={items?.TaskUsers}
        />
      ) : (
        ""
      )}
      {selectedFilterPanelIsOpen && (
        <SelectFilterPanel
          isOpen={selectedFilterPanelIsOpen}
          selectedFilterCallBack={selectedFilterCallBack}
          setSelectedFilterPannelData={setSelectedFilterPannelData}
          selectedFilterPannelData={selectedFilterPannelData}
          portfolioColor={portfolioColor}
        />
      )}
    </>
