...
const EditTaskPopup = (Items: any) => {
  ...
  const [openTeamPortfolioPopup, setOpenTeamPortfolioPopup] =
    ...
    try {
      ...
      if (Items.Items.listId != undefined) {
        extraLookupColumnData = await web.lists
          ...
          .items.select(
            "Project/Id, Project/Title,SmartInformation/Id, AttachmentFiles, Approver/Id, Approver/Title, ClientCategory/Id,ClientCategory/Title, ApproverHistory"
          )
          .top(5000)
          .filter(`Id eq ${Items.Items.Id}`)
          .expand(
            "Project, Approver,SmartInformation, ClientCategory,AttachmentFiles"
          )
          .get();
    ...
    try {
      ...
      if (Items.Items.listId != undefined) {
        smartMeta = await web.lists
          .getById(Items.Items.listId)
          .items.select(
            "Id,Title,PriorityRank,Comments,workingThisWeek,EstimatedTime,EstimatedTimeDescription,waitForResponse,OffshoreImageUrl,OffshoreComments,SiteCompositionSettings,BasicImageInfo,ClientTime,Attachments,AttachmentFiles,Priority,Mileage,CompletedDate,FeedBack,Status,ItemRank,IsTodaysTask,Body,ComponentLink,RelevantPortfolio/Title,RelevantPortfolio/Id,Portfolio/Title,Portfolio/Id,PercentComplete,Categories,TaskLevel,TaskLevel,ClientActivity,ClientActivityJson,StartDate,PriorityRank,DueDate,TaskType/Id,TaskType/Title,Created,Modified,Author/Id,Author/Title,Editor/Id,Editor/Title,TaskCategories/Id,TaskCategories/Title,AssignedTo/Id,AssignedTo/Title,TeamMembers/Id,TeamMembers/Title,ResponsibleTeam/Id,ResponsibleTeam/Title,ClientCategory/Id,ClientCategory/Title"
          )
          .top(5000)
          .filter(`Id eq ${Items.Items.Id}`)
          .expand(
            "AssignedTo,Author,Editor,Portfolio,TaskType,TeamMembers,ResponsibleTeam,TaskCategories,ClientCategory,RelevantPortfolio"
          )
          .get();
      } else {
        smartMeta = await web.lists
          .getByTitle(Items.Items.listName)
          .items.select(
            "Id,Title,PriorityRank,Comments,BasicImageInfo,EstimatedTime,EstimatedTimeDescription,workingThisWeek,OffshoreImageUrl,OffshoreComments,waitForResponse,SiteCompositionSettings,ClientTime,Attachments,AttachmentFiles,Priority,Mileage,CompletedDate,FeedBack,Status,ItemRank,IsTodaysTask,Body,ComponentLink,RelevantPortfolio/Title,RelevantPortfolio/Id,Portfolio/Title,Portfolio/Id,PercentComplete,Categories,TaskLevel,TaskLevel,ClientActivity,ClientActivityJson,StartDate,PriorityRank,DueDate,TaskType/Id,TaskType/Title,Created,Modified,Author/Id,Author/Title,Editor/Id,Editor/Title,TaskCategories/Id,TaskCategories/Title,AssignedTo/Id,AssignedTo/Title,TeamMembers/Id,TeamMembers/Title,ResponsibleTeam/Id,ResponsibleTeam/Title,ClientCategory/Id,ClientCategory/Title"
          )
          .top(5000)
          .filter(`Id eq ${Items.Items.Id}`)
          .expand(
      ...
      smartMeta?.map((item: any) => {
        ...
        if (item.ClientTime != null && item.ClientTime != undefined) {
          ...
          if (Items.Items.siteType == "Shareweb") {
            ...
            if (
              ShareWebConfigData != undefined ||
              ShareWebCompositionStatus == 100
            ) {
              let siteConfigData = JSON.parse(
                ShareWebConfigData != undefined ? ShareWebConfigData : [{}]
              );
              tempData = siteConfigData[0].SiteComposition;
              let siteSeetingJSON = [
                { Manual: true, Proportional: false, Portfolio: false }
              ];
      ...
      if (DataFromCall != undefined) {
        ...
        if (DataFromCall[0].Sitestagging != undefined) {
          let tempSiteComposition: any = JSON.parse(
            DataFromCall[0].Sitestagging != undefined
              ? DataFromCall[0].Sitestagging
              : [{}]
          );
          if (
  ...
  const GetMasterData = async () => {
    try {
      ...
      AllProjects = await web.lists
        .getById(AllListIdData?.MasterTaskListID)
        .items.select(
          "Id,Title,DueDate,TeamMembers/Id,TeamMembers/Title,Parent/Id,Parent/Title,PercentComplete,Status,PriorityRank"
        )
        ...
        await web.lists
          ...
          .then(async (res: any) => {
            ...
            if (usedFor == "Image-Tab") {
              ...
              userSendAttentionEmails = [];
              SiteCompositionPrecentageValue = 0;
              let CalculateStatusPercentage: any = smartMetaCall[0]
                .PercentComplete
                ? smartMetaCall[0].PercentComplete * 100
                : 0;
  ...
  const MakeUpdateDataJSON = async () => {
    ...
    let UpdateDataObject: any = {
      ...
      BasicImageInfo:
        UploadImageArray != undefined && UploadImageArray.length > 0
          ? JSON.stringify(UploadImageArray)
          : JSON.stringify(UploadImageArray),
  ...
  const getTeamConfigData = React.useCallback(
    (teamConfigData: any, Type: any) => {
      if (Type == "TimeSheet") {
  ...
  const copyAndMoveTaskFunction = async (FunctionsType: number) => {
    let CopyAndMoveTaskStatus = confirm(
      `Uploaded Task Images still not moving we are working on it. Click OK if you still would like to proceed without Images`
    );
  ...
  const SaveEstimatedTimeDescription = () => {
    ...
    if (EstimatedTime > 0 && EstimatedDescription?.length > 0) {
      ...
      if (
        EstimatedDescription.length == 0 ||
        EstimatedDescription == undefined
      ) {
  ...
  const onRenderStatusPanelHeader = () => {
    return (
      ...
      >
        <div className="subheading">
          <span className="siteColor">Update Task Status</span>
        </div>
        <Tooltip ComponentId="1683" isServiceTask={ServicesTaskCheck} />
      </div>
    );
  ...
  return (
    ...
    >
                    ...
                    <div className="col mt-2">
                      <div className="input-group taskTime">
                          ...
                          defaultValue={
                            PercentCompleteCheck
                              ? EditData.PercentComplete != undefined
                                ? Number(EditData.PercentComplete).toFixed(0)
                                : null
                              : UpdateTaskInfo.PercentCompleteStatus
                              ? UpdateTaskInfo.PercentCompleteStatus
                              : null
                        ...
                        <span
                          ...
                          title="Status Popup"
                          onClick={() =>
                            openTaskStatusUpdatePopup(EditData, "Status")
                          }
                        >
                          <span
                            title="Edit Task"
      ...
      <Modal
        ...
        isBlocking={AddImageDescriptions}
      ...
      >
        <div className="modal-header mb-1">
          ...
          <span className="mx-1">
            {" "}
            <Tooltip ComponentId="5669" isServiceTask={ServicesTaskCheck} />
          </span>
          <button
            type="button"
            className="btn-close"
