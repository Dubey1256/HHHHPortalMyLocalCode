...
export class TeamConfigurationCard extends React.Component<
  ITeamConfigurationProps,
  ITeamConfigurationState
> {
  ...
  private async loadTaskUsers() {
    ...
    results.forEach(function (item: any) {
      if (item.ItemType != "Group") {
        if (
          self.props.ItemInfo.Services != undefined &&
          self.props.ItemInfo.Services.length > 0
        ) {
    ...
    //         if (
    //             ChildUser.Item_x0020_Cover == undefined ||
    //             ChildUser.Item_x0020_Cover == null
    //         ) {
    //           ChildUser.Item_x0020_Cover.Url =
  ...
  private async GetTaskDetails() {
    ...
    if (this.props.ItemInfo.listId != undefined) {
      taskDetails = await web.lists
        ...
        .items.getById(this.props.ItemInfo.Id)
        .select(
          "ID",
          "Title",
          "AssignedTo/Title",
  ...
  private getChilds(item: any, items: any) {
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      if (
        ...
        if (
          this.props.ItemInfo.Services != undefined &&
          this.props.ItemInfo.Services.length > 0
        ) {
          if (
            childItem.Role != null &&
            childItem.Role.length > 0 &&
            childItem.Role.join(";").indexOf("Service Teams") > -1
          ) {
  ...
  private showComposition() {
    ...
    if (Item.Responsible_x0020_Team != undefined) {
      if (
        self.ResponsibleTeam != undefined &&
        self.ResponsibleTeam.length > 0
      ) {
        let TeamLeaderData = self.getUsersWithImage(
          Item.Responsible_x0020_Team
        );
        ...
        self.ResponsibleTeam = self.getUsersWithImage(
          Item.Responsible_x0020_Team
        );
      }
      self.NewTeamConfigurations.push({
        Title: "Task Leader",
        childs: self.ResponsibleTeam,
      });
    }
    ...
    if (Item.Team_x0020_Members != undefined) {
      if (
        self.TeamMemberUsers != undefined &&
        self.TeamMemberUsers.length > 0
      ) {
        let TeamMemberUsersData = self.getUsersWithImage(
          Item.Team_x0020_Members
        );
    ...
    if (Item.AssignedTo != undefined) {
      if (
        self.AssignedToUsers != undefined &&
        self.AssignedToUsers.length > 0
      ) {
      ...
      for (var i = 0; i < categoryUser.childs.length; i++) {
        if (categoryUser.childs[i].Item_x0020_Cover != undefined) {
          self.TeamMemberUsers.forEach(function (item: any) {
            if (
              categoryUser.childs[i] != undefined &&
              categoryUser.childs[i].AssingedToUserId != undefined &&
              categoryUser.childs[i].AssingedToUserId == item.Id
            ) {
    ...
    for (let i = 0; i < self.AllUsers.length; i++) {
      ...
      if (self.AllUsers[i]) {
        items.forEach(function (item: any) {
          if (
            self.AllUsers[i] != undefined &&
            self.AllUsers[i].AssingedToUserId != undefined &&
            self.AllUsers[i].AssingedToUserId == item.Id
          ) {
      ...
      AllUser.forEach(function (Group: any, index: any) {
        if (Group.childs != undefined && Group.childs.length > 0) {
          Group.childs.forEach(function (user: any, userindex: any) {
            if (
              (user.AssingedToUserId != undefined &&
                user.AssingedToUserId == $data.AssingedToUserId) ||
              (user.Id != undefined && user.Id == $data.Id)
            ) {
            ...
            <div className="border col p-2" ng-show="teamUserExpanded">
              <div className="taskTeamBox">
                {this.state.taskUsers != null &&
                  ...
                  this.state.taskUsers.map((user: any, index: number) => {
                    return (
                      <div
                        ui-on-drop="onDropRemoveTeam($event,$data,taskUsers)"
                        className="top-assign ng-scope"
                      >
                        {user.childs.length > 0 && (
                          <div
                            ng-if="user.childs.length >0"
                            className="team ng-scope"
                          >
                            <label className="BdrBtm">{user.Title}</label>
                            <div className="d-flex User">
                              {user.childs.map((item: any, i: number) => {
                                return (
                                  <div className="marginR41 ng-scope">
                                    {item.Item_x0020_Cover != undefined &&
                                    item.Item_x0020_Cover != null ? (
                                      <span>
                                        <div
              ...
              <div className="row">
                      ...
                      >
                        <div className="p-1">
                          <div
                            ...
                            style={{ height: "30px" }}
                          >
                            {this.state.ResponsibleTeam != null &&
                              this.state.ResponsibleTeam.length > 0 &&
                              this.state.ResponsibleTeam.map(
                      ...
                      >
                        <div className="p-1">
                          <div
                            ...
                            style={{ height: "30px" }}
                          >
                            {this.state.TeamMemberUsers != null &&
                              this.state.TeamMemberUsers.length > 0 &&
                              this.state.TeamMemberUsers.map(
                                (image: any, index: number) => {
                                  return (
                                    <div
                                      className="ProirityAssignedUserPhoto"
                ...
                <div className="col-sm-3">
                  ...
                  >
                    <div className="working-box p-1">
                      ...
                      >
                        {this.state.AssignedToUsers &&
                          this.state.AssignedToUsers.map(
                            (image: any, index: number) => {
                              return (
                                <div
                                  className="ProirityAssignedUserPhoto"
                ...
                <div className="col-sm-2">
                  <div>
                    <div
                      onDrop={(e) =>
                        this.onDropRemoveTeam(e, this.state.taskUsers)
                      }
                      onDragOver={(e) => e.preventDefault()}
                    >
                      <img
                        ...
                        className="width-75"
                        src={
                          this.props?.ItemInfo?.Portfolio_x0020_Type ==
                            "Service" ||
                          (this.props?.ItemInfo?.Services != undefined &&
                            this.props?.ItemInfo?.Services.length > 0)
