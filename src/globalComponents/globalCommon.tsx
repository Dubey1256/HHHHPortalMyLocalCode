...
export const docxUint8Array = async () => {
  ...
  await getData(
    ...
    "e968902a-3021-4af2-a30a-174ea95cf8fa",
    "Id,ID,Title,Configurations&$filter=Title eq 'docxConfig'"
  ).then((data: any) => {
      ...
      .then((client: MSGraphClientV3) => {
        client
          ...
          .get(async (err: any, res: any) => {
            if (err) console.error("MSGraphAPI Error");
            let TeamUser: any[] = [];
            let participants: any = [];
            TeamUser = res?.value;
            let CurrentUserChatInfo = TeamUser.filter((items: any) => {
              if (
                items.userPrincipalName != undefined &&
                currentUser.Email != undefined &&
                items.userPrincipalName.toLowerCase() ==
            ...
            for (let index = 0; index < mention_To?.length; index++) {
              for (
                let TeamUserIndex = 0;
                TeamUserIndex < TeamUser?.length;
                TeamUserIndex++
              ) {
                if (
                  mention_To[index] != undefined &&
                  TeamUser[TeamUserIndex] != undefined &&
                  mention_To[index].toLowerCase() ==
                    TeamUser[TeamUserIndex].userPrincipalName.toLowerCase()
                )
                ...
                if (
                  ...
                  TeamUser[TeamUserIndex] != undefined &&
                  mention_To[index].toLowerCase() ==
                    "stefan.hochhuth@hochhuth-consulting.de" &&
                  TeamUser[TeamUserIndex].id ==
                    "b0f99ab1-aef3-475c-98bd-e68229168489"
                )
                  SelectedUser.push(TeamUser[TeamUserIndex]);
              }
            }
            let obj = {
              ...
              roles: ["owner"],
              "user@odata.bind": `https://graph.microsoft.com/v1.0/users('${currentUser?.ChatId}')`
            };
            ...
            mention_To != undefined && mention_To?.length == 1
              ? (chat_payload.chatType = "oneOnOne")
              : (chat_payload.chatType = "group");
            let new_chat_resp = await client
              .api("/chats")
              .version("v1.0")
              .post(chat_payload);
            ...
            let result = await client
              .api("/chats/" + new_chat_resp?.id + "/messages")
              .post(message_payload);
            return result;
          });
      });
  } catch (error) {
    ...
    if (MainParent != undefined && MainParent.length > 0) {
      if (MainParent != undefined && MainParent.length > 0) {
        MainParent?.forEach((val: any) => {
          ...
          if (
            val.Item_x0020_Type != undefined &&
            val.Item_x0020_Type === "Component"
          ) {
            val.SiteIconTitle = "C";
          }
          if (
            val.Item_x0020_Type != undefined &&
            val.Item_x0020_Type === "SubComponent"
          ) {
            val.SiteIconTitle = "S";
          }
          if (
            val.Item_x0020_Type != undefined &&
            val.Item_x0020_Type === "Feature"
          ) {
            val.SiteIconTitle = "F";
          }
          //val.subRows(val)
...
export const getTaskId = (item: any) => {
  ...
  try {
    if (item != undefined && item.TaskType == undefined) {
      ...
      (item.TaskType.Title == "Task" || item.TaskType.Title == "MileStone") &&
      item.TaskLevel == undefined &&
      item.TaskLevel == undefined
    ) {
      TaskID = "T" + item.Id;
      if (item.TaskType.Title == "MileStone") TaskID = "M" + item.Id;
    } else if (
      item.TaskType != undefined &&
      (item.TaskType.Title == "Activities" ||
        item.TaskType.Title == "Project") &&
      item.TaskLevel != undefined
    ) {
      ...
      if (item?.TaskType?.Title == "Project") TaskID = "P" + item.TaskLevel;
      ...
      item.TaskType != undefined &&
      (item.TaskType.Title == "Workstream" || item.TaskType.Title == "Step") &&
      item.TaskLevel != undefined &&
      item.TaskLevel != undefined
    ) {
      if (
      ...
      if (
        (item?.Component?.length == 0 || item?.Component == undefined) &&
        (item?.Services?.length == 0 || item?.Services == undefined) &&
        item?.Events == undefined
      ) {
        TaskID = "A" + item.TaskLevel + "-W" + item.TaskLevel;
      }
      ...
      if (item.Component != undefined) {
        if (item.Component != undefined && item?.Component?.length > 0) {
          TaskID =
            "CA" + item.TaskLevel + "-W" + item.TaskLevel + "-T" + item.Id;
        }
      }
      if (item.Services != undefined) {
        if (item.Services != undefined && item?.Services?.length > 0) {
          TaskID =
            "SA" + item.TaskLevel + "-W" + item.TaskLevel + "-T" + item.Id;
        }
      }
      if (item.Events != undefined) {
        if (item.Events != undefined && item?.Events?.length > 0) {
          TaskID =
            "EA" + item.TaskLevel + "-W" + item.TaskLevel + "-T" + item.Id;
        }
      }
      if (
        item.Component == undefined &&
...
export const loadTaskUsers = async () => {
  ...
  try {
    ...
    taskUser = await web.lists
      ...
      .items.select(
        "Id,UserGroupId,Suffix,Title,Email,SortOrder,Role,Company,ParentID1,Status,Item_x0020_Cover,AssingedToUserId,isDeleted,AssingedToUser/Title,AssingedToUser/Id,AssingedToUser/EMail,ItemType,Approver/Id,Approver/Title,Approver/Name&$expand=AssingedToUser,Approver"
      )
      .get();
  } catch (error) {
    return Promise.reject(error);
...
export const makePostDataForApprovalProcess = async (postData: any) => {
  ...
  await loadTaskUsers().then(
    function (data) {
      ...
      if (
        postData?.Categories?.toLowerCase().indexOf("approval") > -1 &&
        UserManager != undefined &&
        UserManager?.length > 0
      ) {
        ...
        if (
          postData?.AssignedToId?.results?.length > 0 &&
          UserManager?.length > 0
        ) {
          UserManager.map((ID: any) => {
...
export const GetImmediateTaskNotificationEmails = async (
  item: any,
  ...
  if (
    item != undefined &&
    (item.PercentComplete == 80 || item.PercentComplete == 93)
  ) {
    query =
      "Id,Title,IsTaskNotifications,AssingedToUser/Title,AssingedToUser/EMail,AssingedToUser/Name,AssingedToUser/Id&$expand=AssingedToUser&$filter=TaskStatusNotification eq " +
      item?.PercentComplete +
      "";
  }
  if (
    (item?.PercentComplete == 80 && item?.newCategories == "Immediate") ||
    (item?.PercentComplete == 90 && item?.newCategories == "Immediate") ||
    (item?.PercentComplete == 90 && item?.newCategories == "Email Notification")
  ) {
    query =
      "Id,Title,IsTaskNotifications,AssingedToUser/Title,AssingedToUser/EMail,AssingedToUser/Name,AssingedToUser/Id&$expand=AssingedToUser&$filter=TaskStatusNotification eq " +
      item?.PercentComplete +
      " or AssingedToUser/Id eq " +
      item?.Author?.Id +
      "";
  }
  if (item?.PercentComplete == 5 && item?.newCategories == "Immediate") {
    query =
      "Id,Title,IsTaskNotifications,AssingedToUser/Title,AssingedToUser/EMail,AssingedToUser/Name,AssingedToUser/Id&$expand=AssingedToUser&$filter= AssingedToUser/Id eq " +
      item?.Author?.Id +
      "";
  }
  if (item == undefined) {
    query =
      "Id,Title,IsTaskNotifications,Email,AssingedToUser/Title,AssingedToUser/EMail,AssingedToUser/Name,AssingedToUser/Id&$expand=AssingedToUser&$filter=IsTaskNotifications eq 1";
  }
  if (item?.TeamLeadersId != undefined) {
    ...
    if (item?.TeamLeadersId != undefined) {
      item.TeamLeadersId.map((UserId: any, indexing: any) => {
        ...
        else filter = filter + "AssingedToUser/Id eq " + UserId;
      });
    } else {
      item.TeamLeadersId.map((UserId: any, indexing: any) => {
        if (item.TeamLeadersId.length - 1 != indexing)
    ...
    query =
      "Id,Title,IsTaskNotifications,AssingedToUser/Title,AssingedToUser/EMail,Email,AssingedToUser/Name,AssingedToUser/Id&$expand=AssingedToUser&$filter=" +
      filter;
  } else if (
    item?.TeamLeadersId?.length == 0 &&
    isLoadNotification == "ApprovalMail"
  ) {
    query =
      "Id,Title,IsTaskNotifications,AssingedToUserId,Approver/Title,Approver/EMail,Email,Approver/Name,Approver/Id&$expand=Approver";
  }
  if (query != undefined && query != "") {
    var listID =
      rootsite != undefined
        ? rootsite.TaskUserlistId
        : GlobalConstants.ADMIN_TASK_USERS_LISTID;
    await getData(
      rootsite != undefined ? rootsite.SiteUrl : pageContent?.WebFullUrl,
      listID,
      query
    ).then(
      (data: any) => {
        ...
        if (
          item != undefined &&
          item.TeamLeadersId != undefined &&
          isLoadNotification != undefined &&
          isLoadNotification != "" &&
          isLoadNotification == "ApprovalMail"
        ) {
          ...
          Allusers.map((user: any) => {
            if (user?.Email != null || user?.Email != undefined) {
              ...
              if (
                user.AssingedToUser.EMail != null ||
                user.AssingedToUser.EMail != undefined
              ) {
        ...
        if (
          Allmail == undefined ||
          (Allmail.length == 0 && isLoadNotification == "ApprovalMail")
        )
...
export const getListNameFromItemProperties = (item: any) => {
  ...
  if (metadataType != undefined) listName = metadataType.split(".");
  listName = listName[2];
  if (listName != undefined)
...
export const ConvertLocalTOServerDate = async (
  LocalDateTime: any,
  dtformat: any
) => {
          ...
          async (data: any) => {
            data?.data?.map((item: any) => {
              ...
              if (item.PercentageCompleted != undefined) {
                item.PercentageCompleted = parseInt(
                  (item?.PercentageCompleted).toFixed(0)
                );
              }
              if (item.PercentComplete != undefined) {
                item.PercentComplete = parseInt(
            ...
            if (UpdateItem?.Created != undefined)
              UpdateItem.Created = ConvertLocalTOServerDate(
                UpdateItem.Created,
                "DD-MMM-YYYY"
              );
            else UpdateItem.Created = "";
            if (UpdateItem?.Modified != undefined)
              UpdateItem.Modified = ConvertLocalTOServerDate(
                UpdateItem.Modified,
                "DD-MMM-YYYY"
              );
            ...
            if (item == undefined) {
              ...
              if (
                item?.PercentComplete == 5 &&
                item?.newCategories == "Immediate"
              ) {
                Subject =
                  "[" +
                  item?.siteType +
              ...
              if (
                item?.TeamLeadersId?.length > 0 &&
                item?.CategoriesType == undefined &&
                item?.isApprovalRejection == undefined
              ) {
                Subject =
                  "[" +
                  item?.siteType +
              ...
              if (
                (item != undefined &&
                  ...
                  item.newCategories == undefined) ||
                (item.PercentComplete == 80 &&
                  item.newCategories != undefined &&
                  item.newCategories != "Immediate" &&
                  item.newCategories != "Email Notification")
              ) {
                Subject =
                  "[" +
                  item?.siteType +
              ...
              if (
                item?.CategoriesType?.toLowerCase().indexOf("draft") > -1 ||
                (item?.CategoriesType?.toLowerCase().indexOf("approval") > -1 &&
                  item?.PercentComplete == 1)
              ) {
                CC = [];
                if (
                ...
                TaskDescription =
                  UpdateItem?.Author1 +
                  " has created a Task which requires your Approval.Please take your time and review:";
                if (
                  item?.TotalApprovalTask != undefined &&
                  item?.TotalApprovalTask != 0
                )
                  NoOfApprovalTask =
                    "Please note that you still have " +
                    item?.TotalApprovalTask +
                    " tasks left to approve.You can find all pending approval tasks on your task dashboard or the approval page.";
                TaskDashBoardURl =
                  "https://hhhhteams.sharepoint.com/sites/HHHH/SP/SitePages/TaskDashboard.aspx";
                ApprovalDashboard =
                  "https://hhhhteams.sharepoint.com/sites/HHHH/SP/SitePages/TaskManagement.aspx?SmartfavoriteId=101&smartfavorite=All%20Approval%20Tasks";
              ...
              if (
                ...
                TaskDescriptionStart = "Hi,";
                TaskDescription =
                  "Your task has been approved by " +
                  item?.ApproverName +
                  ", team will process it further. Refer Approval Comments.";
                TaskComments = item?.TaskComments;
                ApprovalRejectionComments =
                  '<tr><td style="border: 1px solid #ccc;background: #f4f4f4;"><b style="font-size: 13px;">Approval Comments:</b> </td><td colspan="7" style="border: 1px solid #ccc;background: #fafafa;"><span style="font-size: 13px; margin-left:13px">' +
                  TaskComments +
                  "</span> </td>" +
                  "</tr>";
              }
              if (
                ...
                TaskDescriptionStart = "Hi,";
                TaskDescription =
                  "Your task has been rejected by " +
                  item?.ApproverName +
                  ". Refer Reject Comments.";
                TaskComments = item.TaskComments;
                ApprovalRejectionComments =
                  '<tr><td style="border: 1px solid #ccc;background: #f4f4f4;"><b style="font-size: 13px;">Rejection Comments:</b> </td><td colspan="7" style="border: 1px solid #ccc;background: #fafafa;"><span style="font-size: 13px; margin-left:13px">' +
                  TaskComments +
                  "</span> </td>" +
                  "</tr>";
              }
              ...
              if (
                ...
                TaskDescription =
                  "Your immediate attention required on this task please review and respond ASAP.";
              }
              //---------
            }
    ...
    if (type != undefined) {
      ...
      if (type == "All") {
        ...
        componentDetails = await web.lists
          ...
          .filter("Portfolio_x0020_Type eq '" + type + "'")
          .top(4999)
          .get();
      }
...
export const GetServiceAndComponentAllData = async (Props: any) => {
  ...
  try {
    ...
    $.each(AllMasterTaskData, function (index: any, result: any) {
      ...
      result.isSelected = Props?.selectedItems?.find(
      ...
      if (result.Item_x0020_Type == "Component") {
        ...
        AllPathGeneratedData = [
          ...AllPathGeneratedData,
          ...groupedResult?.PathArray
        ];
  ...
  console.log(
    "all Service and Coponent data in global common =======",
    AllMasterTaskData
  );
};
...
const componentGrouping = (
  Portfolio: any,
  AllProtFolioData: any,
  path: string = ""
) => {
  let pathArray: any = [];
  Portfolio.subRows = [];
  let subComFeat = AllProtFolioData?.filter(
    (comp: any) => comp?.Parent?.Id === Portfolio?.Id
  );
  ...
  subComFeat?.forEach((subComp: any) => {
    subComp.subRows = [];
    let allFeattData = AllProtFolioData?.filter(
      (elem: any) => elem?.Parent?.Id === subComp?.Id
    );
...
export const GetTaskId = (Item: any) => {
  let taskIds = "";
  if (
    Item?.Portfolio?.PortfolioStructureID != undefined &&
    Item.TaskID != undefined &&
    Item?.ParentTask?.TaskID != undefined
  ) {
    taskIds =
      Item?.Portfolio?.PortfolioStructureID +
      "-" +
      Item.ParentTask?.TaskID +
      "-" +
      Item.TaskID;
  } else if (
    Item?.Portfolio?.PortfolioStructureID != undefined &&
    ...
    taskIds = Item?.ParentTask?.TaskID + "-" + Item.TaskID;
  }
  if (
    Item?.Portfolio?.PortfolioStructureID != undefined &&
    Item.TaskID == undefined &&
    ...
    taskIds = Item?.ParentTask?.TaskID + "-T" + Item.Id;
