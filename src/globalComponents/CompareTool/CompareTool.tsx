...
const CompareTool = (props: any) => {
    ...
    const [SmartMetaDataAllItems, setSmartMetaDataAllItems] = useState([]);
                ...
                await globalCommon.getData(
                    ...
                    if (datas?.length > 0) {
                        ...
                        datas[0].siteUrl = props?.contextValue?.siteUrl,
                            datas[0].listId = props?.contextValue?.MasterTaskListID,
                            datas[0].siteType = "Master Tasks",
                            getDocuments(datas)
                        a.push(...datas);
    ...
    const getDocuments = async (data: any) => {
        try {
            ...
            data[0].tagDoc = items;//{ EncodedAbsUrl: items.EncodedAbsUrl, Title: items?.Title }
            console.log(items);
        } catch (error) {
            console.error(error);
        }
    ...
    const taggedChildItems = (index: any, property: any, value: any) => {
        ...
        if (taggedItems != undefined && (property === 'tagDoc' || property === 'taggedTasks' || property === 'subRows')) {
    ...
    return (
        <>
            <Panel onRenderHeader={onRenderCustomHeaderMain} type={PanelType.large} isOpen={props?.isOpen} isBlocking={false} onDismiss={() => props?.compareToolCallBack("close")}>
                <Modal.Body className="mb-5">
                    <Container fluid className="CompareSmartpopup">
                                ...
                                <div className="SpfxCheckRadio alignCenter">
                                    {data[0]?.tagDoc?.length > 0 && data[0]?.tagDoc?.map((items: any) => {
                                        return (<>
                                            ...
                                            </a><a className='ps-2' href={`${items?.EncodedAbsUrl}?web=1`} target="_blank" data-interception="off"> <span>{items?.Title}</span></a>
                                        </>
                                        )
                                    })}
                                </div>
                        ...
                        <Row className="Metadatapannel">
                            <Col sm="5" md="5" lg="5" className="sit-preview">
                        ...
                        <Row className="Metadatapannel">
                            <Col sm="5" md="5" lg="5">
                                ...
                                <Text>{data[0]?.HelpDescription}</Text> */}
                                <div className="input-group">
            ...
            {
                isPicker?.condition && <ServiceComponentPortfolioPopup
                    props={catItem}
