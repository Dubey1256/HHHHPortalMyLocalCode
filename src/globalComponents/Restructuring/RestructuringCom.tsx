...
const RestructuringCom = (props: any, ref: any) => {
      ...
      props?.allData?.map((obj: any) => {
        ...
        if (obj?.subRows?.length > 0 && obj?.subRows != undefined) {
          obj?.subRows?.map((sub: any) => {
            ...
            const matchingTask = props?.AllMasterTasksData?.find(
              (task: any) => sub?.Portfolio?.Id === task?.Id
            );
            if (matchingTask && portfolioTypeCheck != "") {
            ...
            if (sub?.subRows?.length > 0 && sub?.subRows != undefined) {
              sub?.subRows?.map((feature: any) => {
                ...
                const matchingTask = props?.AllMasterTasksData?.find(
                  (task: any) => feature?.Portfolio?.Id === task?.Id
                );
                if (matchingTask && portfolioTypeCheck != "") {
                  feature.PortfolioTypeCheck =
                    matchingTask?.PortfolioType?.Title;
                ...
                if (
                  ...
                  feature?.subRows?.map((activity: any) => {
                    ...
                    const matchingTask = props?.AllMasterTasksData?.find(
                      (task: any) => activity?.Portfolio?.Id === task?.Id
                    );
                    if (matchingTask && portfolioTypeCheck != "") {
                      activity.PortfolioTypeCheck =
                        matchingTask?.PortfolioType?.Title;
                      ...
                      if (
                        portfolioTypeCheck != "" &&
                        activity?.Item_x0020_Type !== "Task"
                      ) {
                        activity.PortfolioTypeCheck =
                          activity?.PortfolioType?.Title;
                      } else if (
                        portfolioTypeCheck != "" &&
                        activity?.Title == "Others"
                      ) {
                    ...
                    if (
                      ...
                      activity?.subRows?.map((wrkstrm: any) => {
                        ...
                        const matchingTask = props?.AllMasterTasksData?.find(
                          (task: any) => wrkstrm?.Portfolio?.Id === task?.Id
                        );
                        if (matchingTask && portfolioTypeCheck != "") {
                          wrkstrm.PortfolioTypeCheck =
                            matchingTask?.PortfolioType?.Title;
                          ...
                          if (
                            portfolioTypeCheck != "" &&
                            wrkstrm?.Item_x0020_Type !== "Task"
                          ) {
                            wrkstrm.PortfolioTypeCheck =
                              wrkstrm?.PortfolioType?.Title;
                          } else if (
                            portfolioTypeCheck != "" &&
                            wrkstrm?.Title == "Others"
                          ) {
                        ...
                        if (
                          ...
                          wrkstrm?.subRows?.map((task: any) => {
                            ...
                            const matchingTask =
                              props?.AllMasterTasksData?.find(
                                (task: any) => task?.Portfolio?.Id === task?.Id
                              );
                            if (matchingTask && portfolioTypeCheck != "") {
                              task.PortfolioTypeCheck =
                                matchingTask?.PortfolioType?.Title;
        ...
        restructureItem?.map((items: any, length: any) => {
          if (
            ...
            if (checkPortfolioType) {
              ...
              itemTypes = "";
              alert(
                "You can not restructure the items with diffrent portfolio types"
              );
            }
          }
        });
    ...
    if (
      ...
      restructureItem?.map((items: any, length: any) => {
        if (items?.Item_x0020_Type === "Component") {
          ...
          if (
            props?.queryItems?.TaskType?.Id === 1 ||
  ...
  const makeMultiSameTask = () => {
    ...
    if (restructureItem[0]?.Item_x0020_Type == "Task") {
      ...
      let TaskId =
        newItemBackUp?.TaskID == undefined
          ? newItemBackUp?.TaskID
          : newItemBackUp?.PortfolioStructureID;
          ...
          .then(async (res: any) => {
            ...
            latestCheckedList?.map((items: any) => {
              (items.ParentTask = { Id: ParentTask_Portfolio }),
                (items.Portfolio = {
                  Id: ParentTask_Portfolio,
                  ItemType:
                    RestructureChecked[0]?.TaskType?.Title == undefined
                      ? RestructureChecked[0]?.Item_x0020_Type
                      : RestructureChecked[0]?.TaskType?.Title,
                  Title: restructureItem[0]?.Title
                }),
                (items.TaskLevel = TaskLevel),
                (items.TaskType = {
                  Id: RestructureChecked[0]?.TaskType?.Id,
                  Level: RestructureChecked[0]?.TaskType?.Level,
                  Title: RestructureChecked[0]?.TaskType?.Title
                }),
                (items.TaskID =
                  RestructureChecked[0]?.TaskType?.Id == 2
                    ? TaskId + "-" + "T" + RestructureChecked[0]?.Id
    ...
    array.forEach((obj: any) => {
      ...
      if (obj.TaskID === item.TaskID && obj.Id === item.Id) {
        object = {
          ...
          Item_x0020_Type: obj.Item_x0020_Type,
          siteIcon:
            obj.SiteIconTitle === undefined ? obj.SiteIcon : obj.SiteIconTitle
        };
  ...
  const trueTopIcon = (items: any) => {
    if (props?.queryItems != undefined && props?.queryItems != null) {
      props?.queryItems?.Item_x0020_Type == "Component"
        ...
        : props?.queryItems?.Item_x0020_Type == "Activity"
      ...
      var postData: any = {
        ...
        TaskID:
          TaskTypeId == 2
            ...
            : TaskTypeId == 1
            ? TaskId + "-" + "A" + TaskLevel
            : TaskTypeId == 3 && newItemBackUp?.Item_x0020_Type == "Task"
            ? TaskId + "-" + "W" + TaskLevel
            : TaskId + "-" + TaskID
      };
              ...
              {RestructureChecked?.map((items: any) => (
                <span>
                  ...
                  {items?.Item_x0020_Type === "Component" ? (
                    ...
                    <div className="Dyicons">{items?.siteIcon}</div>
                  )}
                  <a
                    data-interception="off"
                    target="_blank"
      ...
      {ResturuningOpen === true && restructureItem?.length > 1 ? (
        ...
        >
          <div>
            ...
            <span>
              {NewArrayBackup[0]?.siteIcon?.length === 1 ? (
                <span className="Dyicons ">{NewArrayBackup[0]?.siteIcon}</span>
              ) : (
                <span>
                  <img
                    width={"25px"}
          ...
          <footer className="mt-2 text-end">
            <button
              className="me-2 btn border-primary"
              onClick={() => setResturuningOpen(false)}
            >
              Cancel
            </button>
