...
export default function ProjectOverview(props: any) {
    ...
    const GetMasterData = async () => {
        if (AllListId?.MasterTaskListID != undefined) {
            try {
                ...
                let AllProject = AllProjectItems?.filter((item: any) => item?.Item_x0020_Type == "Project")

                AllProject = sortOnPriority(AllProject)
                let flatDataProjects = globalCommon.deepCopy(AllProject);
                let AllProjectDataWithAWT = globalCommon.deepCopy(AllProject);
    ...
    const LoadAllSiteTasks = async () => {
        if (siteConfig?.length > 0) {
            try {
                var AllTask: any = [];
                let smartmeta: any = [];
                smartmeta = await globalCommon?.loadAllSiteTasks(AllListId)
    ...
    const customTableHeaderButtons = (
        <button type="button" className={`btn btn-${workingTodayFiltered ? 'primary' : 'grey'}`} onClick={() => { setWorkingTodayFiltered(!workingTodayFiltered) }}> Working-Today </button>
    )
