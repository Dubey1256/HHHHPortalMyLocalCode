...
const inlineEditingcolumns = (props: any) => {
    ...
    React.useEffect(() => {
        ...
        let taskDueDate= props?.item?.DueDate != undefined?Moment(new Date(props.item.DueDate)).format('YYYY-MM-DD'):'';
        setDueDate({...dueDate, editDate:taskDueDate})
        GetSmartMetadata();
    }, [])
...
{/* Panel to edit due-date */}
            ...
            >
                <div className={ServicesTaskCheck ? "serviepannelgreena" : ""} >
                    ...
                    <input className="form-check-input p-3 w-100"
                       type='date' 
                      value={dueDate.editDate!=undefined?Moment(new Date(dueDate.editDate)).format('YYYY-MM-DD'):''}
            ...
            {props?.columnName == 'DisplayDueDate' ?  <span onClick={() =>setDueDate({...dueDate, editPopup:true})} style={{ display: "block", width: "100%" }}>&nbsp;{props?.item?.DisplayDueDate}</span>    : " "
            }
