            ...
            map(siteConfig, async (config: any) => {
                ...
                if (AllTasksMatches != undefined && AllTasksMatches.length > 0) {
                    $.each(AllTasksMatches, function (index: any, item: any) {
                        ...
                        if(item.Portfolio_x0020_Type === "Service" || item.Services.length > 0){
                            item.dynamicColor='colorService'
                            item.dynamicBgColor='colorServiceBgColor'
                        }else if(item.Portfolio_x0020_Type === "Component" || item.Component.length > 0){
                            item.dynamicColor='colorComponent'
                            item.dynamicBgColor='colorComponentBgColor'
                        }
    ...
    const GetComponents = async () => {
        ...
        componentDetails.forEach((result: any) => {
            ...
            if(result.Portfolio_x0020_Type ==="Service"){
                result.dynamicColor='colorService'
                result.dynamicBgColor='colorServiceBgColor'
            }else if(result.Portfolio_x0020_Type === "Component"){
                result.dynamicColor='colorComponent'
                result.dynamicBgColor='colorComponentBgColor'
            }
                    ...
                    <div className="d-flex">
                        <>
                            {row.getCanExpand() ? (
                                ...
                                >
                                    {row.getIsExpanded() ? <FaChevronDown className={ IsUpdated != "Service Portfolio" && IsUpdated != "Component Portfolio" ? row?.original?.dynamicColor: ''}/> : <FaChevronRight className={ IsUpdated != "Service Portfolio" && IsUpdated != "Component Portfolio" ? row?.original?.dynamicColor: ''}/>}
                                </span>
                            ) : (
                                ""
                            )}{" "}
                                ...
                                <>
                                    {row?.original?.Title != "Others" ? (
                                        <div className={
                                            ...
                                            (IsUpdated !== "Service Portfolio" && IsUpdated !== "Component Portfolio")
                                              ? row?.original?.dynamicBgColor + " ml-12 Dyicons"
                                              : row?.original?.Item_x0020_Type === "Feature" ||
                                                (IsUpdated !== "Service Portfolio" && IsUpdated !== "Component Portfolio")
                                              ? row?.original?.dynamicBgColor + " ml-24 Dyicons"
                                              : row?.original?.SharewebTaskType?.Title === "Activities" ||
                                                (IsUpdated !== "Service Portfolio" && IsUpdated !== "Component Portfolio")
                                              ? row?.original?.dynamicBgColor + " ml-36 Dyicons"
                                              : row?.original?.SharewebTaskType?.Title === "Workstream" ||
                                                (IsUpdated !== "Service Portfolio" && IsUpdated !== "Component Portfolio")
                                              ? row?.original?.dynamicBgColor + " ml-48 Dyicons"
                                              : row?.original?.SharewebTaskType?.Title === "Task" ||
                                                (IsUpdated !== "Service Portfolio" && IsUpdated !== "Component Portfolio")
                                              ? row?.original?.dynamicBgColor + " ml-60 Dyicons"
                                              : "Dyicons"
                                          }>
                ...
                cell: ({ row, column, getValue }) => (
                    <>
                        {row?.original?.siteType == "Master Tasks" && row?.original?.Title !== "Others" && (
                            <a data-interception="off" target="_blank"   className={ IsUpdated != "Service Portfolio" && IsUpdated != "Component Portfolio" ? row?.original?.dynamicColor + "  hreflink serviceColor_Active": 'hreflink serviceColor_Active'} href={ContextValue.siteUrl + "/SitePages/Portfolio-Profile.aspx?taskId=" + row?.original?.ID} >
                                <HighlightableCell value={getValue()} searchTerm={column.getFilterValue() != undefined ? column.getFilterValue() : globalFilterHighlited} />
                            </a>
                        )}
                        {row?.original?.siteType != "Master Tasks" && row?.original?.Title !== "Others" && (
                            <a data-interception="off" target="_blank" className={ IsUpdated != "Service Portfolio" && IsUpdated != "Component Portfolio" ? row?.original?.dynamicColor + "  hreflink serviceColor_Active": 'hreflink serviceColor_Active'} onClick={(e) => EditData(e, row?.original)}
                                                        ...
                                                        return (
                                                            <tr className={row?.original?.lableColor} key={row.id} >
                                                                {row.getVisibleCells().map((cell: any) => {
                                                                    return (
                                                                        <td className={row?.original?.Portfolio_x0020_Type === "Service" ?  `${row?.original?.boldRow} ${row?.original?.dynamicColor}` : row?.original?.boldRow} key={cell.id}>
                                                                            {flexRender(
