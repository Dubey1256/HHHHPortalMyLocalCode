...
import GlobalCommanTable from '../../smartmetadataportfolio/components/GlobalCommanTableSmartmetadata';
...
export default function DocumentSearchPage(Props: any) {
    ...
    const LoadDocs = () => {
        ...
        web.lists.getById(PageContext.DocumentListId).items.select('Id,Title,Year,File_x0020_Type,FileLeafRef,FSObjType,Body,FileDirRef,Created,Modified,Author/Id,Author/Title,Editor/Id,Editor/Title,EncodedAbsUrl', 'Portfolios/Id', 'Portfolios/Title').filter('FSObjType eq 0').expand('Author,Editor,Portfolios').getAll()
            .then((response: any) => {
                try {
    ...
    return (
        ...
        <> {AllDocs && <div>
            ...
            {/* {isEditModalOpen ? <DocumentPopuppagecontext={PageContext} Id={selectedItemId} /> : ''} */}
            {isEditModalOpen ?
                <DocumentPopup
                    Item={selectedItemId}
                    closeEditPopup={closeEditPopup}
                    editData={selectedItemId}
                    AllListId={PageContext}
