...
const ImageViewPanel = (props: any) => {
  ...
  const [prosConsStatus, setProsConsStatus] = useState({
    status: false,
    index: 0,
    slider: false,
    single: false,
  ...
  const [updateComment, setUpdateComment]: any = useState({
    selectedData: {},
    CommentIndex: 0,
    commentData: "",
    openPopup: false,
  ...
  const openImageSection = (selectedTitle: any) => {
    ...
    if (checkedImageData?.length == 2) {
      copyAllImage = copyAllImage?.filter(
        (data: any) => data?.ImageName != props?.checkedImageData[0]?.ImageName
      );
    }
  ...
  useEffect(() => {
    if (props?.AllImageData?.length > 0) {
      ...
      if (props?.checkedImageData?.length > 0) {
        SetCheckedImageData(
          JSON.parse(JSON.stringify(props?.checkedImageData))
        );
        SetRightSectionImage(
          JSON.parse(JSON.stringify(props?.checkedImageData))
        );
        if (props?.checkedImageData?.length == 1) {
          ...
          copyAllImage = copyAllImage?.filter(
            (data: any) =>
              data?.ImageName != props?.checkedImageData[0]?.ImageName
          );
  ...
  const changeFunction = (value: any, selectedData: any, label: any) => {
    ...
    if (label == "Rating") {
      selectedData.ImageRating =
        value === selectedData?.ImageRating ? 0 : value;
    } else if (label == "Notes") {
    ...
    let selectedIndex = CopyAllImageData.findIndex(
      (item: any) => item.ImageName === selectedData?.ImageName
    );
    let rightSectionIndex = rightSectionImage?.findIndex(
      (item: any) => item.ImageName === selectedData?.ImageName
    );
  ...
  const handleCheckboxImagChange = (isChecked: any, data: any) => {
    if (isChecked) {
      ...
      SetCheckedImageData(
        checkedImageData.filter(
          (item: any) => item.ImageName !== data.ImageName
        )
      );
    }
  };
  ...
  const saveImageView = async () => {
    ...
    const i = await web.lists
      ...
      .items.getById(props?.taskData?.Id)
      .update({
      ...
      var temp: any = {
        AuthorImage:
          props?.currentUser != null && props?.currentUser?.length > 0
            ? props?.currentUser[0]["userImage"]
            : "",
        AuthorName:
          props?.currentUser != null && props?.currentUser?.length > 0
            ? props?.currentUser[0]["Title"]
            : "",
  ...
  const updateCommentFunction = () => {
    ...
    if (txtComment != "") {
      ...
      let selectedIndex = CopyAllImageData.findIndex(
        (item: any) => item.ImageName === updateComment?.selectedData?.ImageName
      );
      let rightSectionIndex = rightSectionImage?.findIndex(
        (item: any) => item.ImageName === updateComment?.selectedData?.ImageName
      );
      if (rightSectionIndex != undefined) {
        RightImageSection[rightSectionIndex].Comments[
      ...
      if (selectedIndex != undefined) {
        CopyAllImageData[selectedIndex].Comments[updateComment?.CommentIndex] =
          temp;
      }
      setAllImageData(CopyAllImageData);
      closeEditPopup();
  ...
  const clearComment = (
    ...
    if (useFor == "Comment") {
      ...
      if (rightSectionIndex != undefined) {
        RightImageSection[rightSectionIndex].Comments[
          index
        ].ReplyMessages.splice(replyindex, 1);
        SetRightSectionImage(RightImageSection);
        checkDataImage = RightImageSection;
      }
      // if (selectedIndex != undefined) {
  ...
  const shortByFunction = (selectedShortBy: any) => {
    ...
    if (selectedShortBy == "Rating(L-H)") {
      ...
      RightImageSection.sort(
        (a: any, b: any) =>
          new Date(a.UploadeDate).getTime() - new Date(b.UploadeDate).getTime()
      );
    } else if (selectedShortBy == "Date(New-Old)") {
      RightImageSection.sort(
        (a: any, b: any) =>
          new Date(b.UploadeDate).getTime() - new Date(a.UploadeDate).getTime()
      );
    } else if ((selectedShortBy = "Favorite")) {
      RightImageSection = RightImageSection.filter(
        (item: any) => item.fillHeart === true
      );
    }
  ...
  const FilterByRating = (value: any) => {
    ...
    if (value == 5) {
      RightImageSection = RightImageSection.filter(
        (item: any) => item.ImageRating === 5
      );
    } else {
      RightImageSection = RightImageSection.filter(
        (item: any) => item.ImageRating >= value
      );
    }
    SetRightSectionImage([...RightImageSection]);
    setIsPopoverFilterOpen(false);
        ...
        <div className="carouselSlider">
          <Slider ref={(slider) => (sliderRef = slider)} {...settings}>
            {allImageData?.map((slide: any, index: any) => (
              <div
                key={index}
                className={slide?.Exclude == true ? "faded" : ""}
              >
                <img
                  src={slide?.ImageUrl}
                  loading="lazy"
                  className={`h-full w-full  ${
                    slide?.fillHeart && "borderFavImage"
                  }`}
                />
                <div>
                  {checked && (
                    <div className="belowImageSection">
                      <div
                        className="alignCenter justify-content-between"
                        style={{ margin: "8px 0px" }}
                      >
                        <div className="starSection">
                          <Rating
                            initialRating={
                              slide?.ImageRating != undefined
                                ? slide?.ImageRating
                                : 0
                            }
                            ...
                            onChange={(rate: any) =>
                              changeFunction(rate, slide, "Rating")
                            }
                          />
                        </div>
                        <div className="alignCenter">
                          {slide?.Exclude == undefined ||
                          slide?.Exclude == false ? (
                            <a
                              className="alignCenter mx-2"
                              onClick={() =>
                                changeFunction("Exclude", slide, "Exclude")
                              }
                            >
                              <span className="svg__icon--cross svg__iconbox mini me-1"></span>
                              Exclude
                            </a>
                          ) : (
                            <a
                              className="alignCenter mx-2 RestoreImage"
                              onClick={() =>
                                changeFunction("Restore", slide, "Restore")
                              }
                            >
                              <span className="svg__icon--refresh svg__iconbox me-1"></span>
                              Restore
                            </a>
                          )}
                          <a
                            className="alignCenter imageFavorite"
                            onClick={() =>
                              changeFunction("fillHeart", slide, "fillHeart")
                            }
                          >
                            {slide?.fillHeart ? (
                              <BsFillHeartFill className="me-2 fillHeart" />
                            ) : (
                      ...
                      <div className="noteSection">
                        <div className="alignCenter justify-content-between">
                          ...
                          <div className="alignCenter">
                            <a
                              className="alignCenter mx-2 siteColor"
                              onClick={() =>
                                setProsConsStatus({
                                  ...prosConsStatus,
                                  status: true,
                            ...
                            >
                              {(prosConsStatus?.status &&
                                prosConsStatus?.index == index &&
                                prosConsStatus?.slider &&
                                prosConsStatus?.single == false) ||
                              slide?.ImagePros != undefined ? (
                                <svg
                                  xmlns="
                            ...
                            <a
                              className="alignCenter siteColor"
                              onClick={() =>
                                setCommentStatus({
                                  ...commentStatus,
                                  status: true,
                        ...
                        <div className="NotesSection">
                          <textarea
                            className="w-100"
                            onChange={(e) =>
                              changeFunction(e?.target?.value, slide, "Notes")
                            }
                            value={slide?.ImageNotes}
                          ></textarea>
                        </div>
                        {commentStatus?.status &&
                          commentStatus?.index === index &&
                          commentStatus?.slider &&
                          commentStatus?.single == false && (
                            <div className="align-items-center d-flex mb-2">
                              ...
                              <button
                                type="button"
                                className="btn btn-primary btnCol ms-2"
                                onClick={() => PostButtonClick(slide)}
                              >
                                Post
                              </button>
                            </div>
                        ...
                        <div className="AddComment">
                          <div className="SpfxCheckRadio m-0">
                            <div className="col">
                                      ...
                                      <>
                                        ...
                                        <div
                                          style={{
                                            display:
                                              k > 1 &&
                                              (slide?.showMore == undefined ||
                                                slide?.showMore == false)
                                                ? "none "
                                                : "block",
                                          }}
                                          className={
                                            fbComment.isShowLight !=
                                              undefined &&
                                            fbComment.isApprovalComment
                                              ? `col bg-f5f5 p-2  my-1 ${fbComment.isShowLight}`
                                              : "col bg-f5f5 p-2  my-1"
                                          }
                                          title={
                                            fbComment.isShowLight != undefined
                                              ? fbComment.isShowLight
                                              : ""
                                          }
                                        >
                                          <div>
                                                    ...
                                                    >
                                                      ...
                                                      >
                                                        ...
                                                        <div className="footer text-end">
                                                          <button
                                                            className="btnCol btn me-2 btn-primary"
                                                            onClick={() =>
                                                              PostReplyComment(
                                                                slide,
                                                                k
                                                          ...
                                                          <button
                                                            className="btnCol btn btn-default"
                                                            onClick={() =>
                                                              setIsPopoverReplyOpen(
                                                                ""
                                                              )
                                            ...
                                            <div className="col-12 ps-3 pe-0 mt-1">
                                              {fbComment?.ReplyMessages !=
                                                undefined &&
                                                fbComment?.ReplyMessages
                                                  .length > 0 &&
                                                fbComment?.ReplyMessages?.map(
                                                  (
                                                    replymessage: any,
                                                    index: any
                                                  ) => {
                              ...
                              {slide?.Comments != null &&
                                slide?.Comments?.length > 2 && (
                                  <button
                                    type="button"
                                    className="btn btn-primary btnCol"
                                    onClick={() =>
                                      changeFunction(true, slide, "showMore")
                                    }
                                  >
                                    See More
                                  </button>
  ...
  const singleImageView = (slide: any, index: any) => {
    return (
      <div className={slide?.Exclude == true ? "faded" : ""}>
        <img
          src={slide?.ImageUrl}
          className={`w-100 ${slide?.fillHeart && "borderFavImage"}`}
        />
        <div>
          {checked && (
            <div className="belowImageSection">
              ...
              >
                <div className="starSection">
                  <Rating
                    initialRating={
                      slide?.ImageRating != undefined ? slide?.ImageRating : 0
                    }
                    emptySymbol={<MdOutlineStarBorder />}
                    fullSymbol={<MdOutlineStar />}
                    onChange={(rate: any) =>
                ...
                <div className="alignCenter">
                  {slide?.Exclude == undefined || slide?.Exclude == false ? (
                    <a
                      className="alignCenter mx-2"
                      onClick={() =>
                        changeFunction("Exclude", slide, "Exclude")
                    ...
                    >
                      <span className="svg__icon--cross svg__iconbox me-1 mini"></span>
                      Exclude
                    </a>
                  ) : (
                    <a
              ...
              <div className="noteSection">
                <div className="alignCenter justify-content-between">
                  ...
                  <div className="alignCenter">
                    ...
                    >
                      {(prosConsStatus?.status &&
                        prosConsStatus?.index == index &&
                        prosConsStatus?.slider == false &&
                        prosConsStatus?.single) ||
                      slide?.ImagePros != undefined ? (
                        <svg
                          xmlns="
                                                        http://www.w3.org/2000/svg"
                ...
                {commentStatus?.status &&
                  commentStatus?.index === index &&
                  commentStatus?.slider == false &&
                  commentStatus?.single && (
                    <div className="align-items-center d-flex mb-2">
                      <textarea
                        id="txtComment"
                  ...
                  <div className="ProsConsSection">
                    <div className="mt-2">
                      ...
                      <textarea
                        ...
                        style={{ backgroundColor: "#DBEDDB" }}
                        onChange={(e) =>
                          changeFunction(e?.target?.value, slide, "Pros")
                        }
                        value={slide?.ImagePros}
                      ></textarea>
                    </div>
                    <div className="mt-2">
                      ...
                      <textarea
                        ...
                        style={{ backgroundColor: "#FFEAEA" }}
                        onChange={(e) =>
                          changeFunction(e?.target?.value, slide, "Cons")
                        }
                        value={slide?.ImageCons}
                      ></textarea>
                    </div>
                ...
                <div className="AddComment">
                  <div className="SpfxCheckRadio m-0">
                    <div className="col">
                      {slide?.Comments != null &&
                        slide?.Comments?.length > 0 &&
                        slide?.Comments?.map((fbComment: any, k: any) => {
                          return (
                            <div
                              style={{
                                display:
                            ...
                            >
                              <div className="">
                                <div className="d-flex p-0">
                                  <div className="col-1 p-0 wid30">
                                    {fbComment?.AuthorImage != undefined &&
                                    fbComment?.AuthorImage != "" ? (
                                      <img
                                        className="workmember hreflink "
                                        onClick={() =>
                                      ...
                                      <span
                                        ...
                                        title={
                                          fbComment?.AuthorName != undefined
                                            ? fbComment?.AuthorName
                                            : "Default user icons"
                                        }
                                        className="alignIcon svg__iconbox svg__icon--defaultUser"
                                  ...
                                  <div className="col pe-0">
                                        ...
                                        <a
                                          ...
                                          onClick={() =>
                                            setUpdateComment({
                                              ...
                                              selectedData: slide,
                                              CommentIndex: k,
                                              commentData: fbComment?.Title,
                                              openPopup: true,
                                              status: "commentUpdate",
                                            })
                                          }
                                        >
                                          <span className="svg__iconbox svg__icon--edit"></span>
                                    ...
                                    <div>
                                      <span>{fbComment?.Title}</span>
                                    </div>
                                  </div>
                                </div>
                                <div className="col-12 ps-3 pe-0 mt-1">
                                  {fbComment?.ReplyMessages != undefined &&
                                    fbComment?.ReplyMessages.length > 0 &&
                                    fbComment?.ReplyMessages?.map(
                                      (replymessage: any, index: any) => {
                                        return (
                                          <div className="d-flex border ms-3 p-2  mb-1">
                                            <div className="col-1 p-0 wid30">
                                              {replymessage?.AuthorImage !=
    ...
    <>
      ...
      >
        <div className="mt-2 d-flex mb-5">
          <div
            className="Taskaddcomment UXImageLeftSection"
            style={{ display: hideLeftSection ? "none" : "block" }}
          >
            {allImageData?.length > 0 && (
              <div className="p-0">
                <label className="form-label alignCenter mb-2 full-width fw-bold">
                  Images
                  <div className="alignCenter ml-auto gap-1">
                    ...
                    >
                      <span
                        onClick={() => openImageSection("fullScreen")}
                        className={`svg__iconbox svg__icon--${
                          checkedImageData?.length <= 1
                            ? "FilledfullScreen siteColor"
                            : "fullScreen"
                        }`}
                      ></span>
                    </Tooltip>
                    ...
                    >
                      <span
                        onClick={() => openImageSection("compare2")}
                        className={`svg__iconbox svg__icon--${
                          checkedImageData?.length == 2
                            ? "Filledcompare2 siteColor "
                            : "compare2"
                        }`}
                      ></span>
                    </Tooltip>
                    ...
                    >
                      <span
                        onClick={() => openImageSection("compareSeveral")}
                        className={`svg__iconbox svg__icon--${
                          checkedImageData?.length == 3 ||
                          checkedImageData?.length == 4
                            ? "FilledcompareSeveral siteColor"
                            : "compareSeveral"
                        }`}
                      ></span>
                    </Tooltip>
                    ...
                    >
                      <span
                        onClick={() => openImageSection("viewAll")}
                        className={`svg__iconbox svg__icon--${
                          checkedImageData?.length > 4
                            ? "FilledviewAll siteColor"
                            : "viewAll"
                        }`}
                      ></span>
                    </Tooltip>
                    <span
                      onClick={() => SetHideLeftSection(true)}
                      className="svg__iconbox svg__icon--arrowCollapse"
                    ></span>
                  </div>
                </label>

                {allImageData != null &&
                  allImageData?.map((imgData: any, i: any) => {
                    const isChecked = checkedImageData.some(
                      (item: any) => item.ImageName === imgData.ImageName
                    );
                    return (
                      <div className="taskimage p-0 mb-3">
          ...
          <div
            className="UXImageRightSection"
            style={{ width: hideLeftSection ? "100%" : "" }}
          >
            {openImageRightSection && (
              <div className="bg-white p-0 mb-3">
                <div className="alignCenter justify-content-between mb-1">
                  <div className="toggleButon alignCenter">
                    {hideLeftSection && (
                      <span
                        onClick={() => SetHideLeftSection(false)}
                        className="openLeftSection svg__iconbox svg__icon--arrowExpand"
                      ></span>
                    )}
                  ...
                  <span className="alignCenter ml-auto">
                    <div
                      className={`${
                        rightSectionImage?.length == 1 &&
                        shortByFilterSelection == false
                          ? "alignCenter me-5 DisableFilterSorting"
                          : "alignCenter me-5"
                      }`}
                    >
                      <Popover
                        withArrow
                        open={isPopoverShortByOpen}
                        onOpenChange={(e, data) =>
                          setIsPopoverShortByOpen(data.open)
                        }
                      >
                        <PopoverTrigger disableButtonEnhancement>
                          <span className="alignCenter  me-4">
                            {" "}
                            <span
                        ...
                        <PopoverSurface tabIndex={-1}>
                          <div>
                            <div className="Filterhead m-0 d-flex justify-content-between fw-bold ">
                              Sort by
                              <span
                                className="svg__iconbox svg__icon--cross dark"
                                onClick={() => setIsPopoverShortByOpen(false)}
                              ></span>
                            </div>
                            <div className="mt-2">
                              <div
                                onClick={() => shortByFunction("Rating(L-H)")}
                                className="hreflink"
                              >
                                Rating (Lowest to Highest)
                              </div>
                              <div
                                onClick={() => shortByFunction("Rating(H-L)")}
                                className="my-2 hreflink"
                              >
                                Rating (Highest to Lowest)
                              </div>
                              <div
                                onClick={() => shortByFunction("Date(Old-New)")}
                                className="hreflink"
                              >
                                Date (Oldest to Newest)
                              </div>
                              <div
                                onClick={() => shortByFunction("Date(New-Old)")}
                      ...
                      <Popover
                        withArrow
                        open={isPopoverFilterOpen}
                        onOpenChange={(e, data) =>
                          setIsPopoverFilterOpen(data.open)
                        }
                      >
                        <PopoverTrigger disableButtonEnhancement>
                          <span className="alignCenter">
                            <span
                              className="svg__iconbox svg__icon--filter"
                              title="Filter"
                            ></span>
                            Filter
                          </span>
                        ...
                        <PopoverSurface tabIndex={-1}>
                          <div>
                            <div className="Filterhead m-0 d-flex justify-content-between fw-bold ">
                              ...
                              <span
                                className="svg__iconbox svg__icon--cross dark"
                                onClick={() => setIsPopoverFilterOpen(false)}
                              ></span>
                            </div>
                            <div className="mt-2">
                              <label
                                onClick={() => shortByFunction("Favorite")}
                                className="label-hover showFav"
                              >
                                Show Favorite
                              </label>
                              <div className="ratingFilter">
                                <label className="mt-1 label-hover">
                                  Show Rating By Stars
                                </label>
                    ...
                    {checkedImageData?.length <= 2 && (
                      <div className="playpausebutton">
                        <span
                          onClick={previous}
                          className="svg__icon--arrowLeft svg__iconbox"
                        ></span>
                        <span
                          className="svg__icon--arrowRight svg__iconbox"
                          onClick={next}
                        ></span>
                      </div>
                    )}
                      ...
                      return (
                        <div
                          className={
                            hideLeftSection
                              ? `${
                                  checkData?.Exclude != undefined &&
                                  checkData?.Exclude == true
                                    ? " col-sm-4 faded"
                                    : " col-sm-4"
                                }`
                              : `${
                        ...
                        >
                          {singleImageView(checkData, index)}{" "}
                        </div>
                      );
                    })}
                  </div>
        ...
        <footer className="bg-f4 fixed-bottom p-3 text-end">
          ...
          <button
            ...
            onClick={() => props?.SetOpenComparePopup(false)}
          >
            Cancel
          </button>
        </footer>
          ...
          >
            ...
            <footer className="modal-footer mt-2">
              <button
                ...
                onClick={(e) => updateCommentFunction()}
              >
                Save
              </button>
              <button className="btn btn-default ms-1" onClick={closeEditPopup}>
                Cancel
              </button>
            </footer>
          </Panel>
