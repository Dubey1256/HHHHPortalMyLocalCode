...
const SmartTimeTotalFunction = (item: any) => {
  ...
  const EditData = async (items: any) => {
    ...
    var select =
      "Id,Title,TaskDate,Created,Modified,TaskTime,Description,SortOrder,AdditionalTimeEntry,AuthorId,Author/Title,Editor/Id,Editor/Title,Category/Id,Category/Title,TimesheetTitle/Id,TimesheetTitle/Title&$expand=Editor,Author,Category,TimesheetTitle&$filter=" +
      filteres +
      "";
    var count = 0;
      ...
      await $.ajax({
        ...
        success: function (data) {
          ...
          if (data?.d?.results != undefined && data?.d?.results?.length > 0) {
            ...
            if (
              AllTimeSpentDetails != undefined &&
              AllTimeSpentDetails.length > 0
            ) {
              getStructureData();
            }
          }
  ...
  const getStructureData = function () {
    ...
    console.log("All Time", AllTimeSpentDetails);
    ...
    console.log("hover Array", hoversmartArray);
    ...
    newArray.forEach((data: any) => {
      data.additionaltime2.sort((a: any, b: any) => {
      ...
      newArray.map((items: any) => {
        ...
        if (items.additionaltime2.length > 0) {
          $.each(items.additionaltime2, function (index: any, tempItem: any) {
            ...
            if (
              tempItem.TaskTimeInMin != undefined ||
              tempItem.TaskTimeInMin != null
            ) {
  ...
  return (
    <>
      ...
      <span className="openhoverpopup hoverimg">
        ...
        <div
          className={
            additionalTime?.length > 0
              ? "hoverpopup overlay"
              : "hoverpopup overlay blankTooltip"
          }
        >
          ...
          <div className="hoverpopupbody">
            <table className="table mb-0">
              {additionalTime.length > 0 ? (
                <tbody>
                  {additionalTime.length > 0 &&
                    additionalTime.map((items: any, index: any) => {
                      return (
                        <>
                              ...
                              <div className="alignCenter">
                                ...
                                <span className="ms-8">
                                  <SmartTooltipComponent
                                    items={items}
                                    allTaskUsers={allTaskUsers}
                                    siteUrl={item?.AllListId?.siteUrl}
                                  />
                                </span>
                              </div>
                            </td>
                            <td>
                              Total Time {items.hoverTime.toFixed(2)}
                              <span className="mx-1">
                                {items.hoverTime > 1 ? "hours" : "hour"}
                              </span>
                            </td>
                          </tr>

                          {items?.additionaltime2?.length > 0 &&
                            items?.additionaltime2?.map((details: any) => {
                              return (
                                <>
                                  {" "}
                                  <tr>
                ...
                <>
                  <div className="noData">No Time Entry</div>
                </>
              )}
            </table>
      ...
      {isTimeEntry ? (
        <TimeEntry
          ...
          parentCallback={ComponentCallBack}
        />
      ) : (
        ""
      )}
