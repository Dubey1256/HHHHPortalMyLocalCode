...
const RelevantDocuments = (props: any, ref: any) => {
  ...
  const loadAllSitesDocuments = async () => {
    let query =
      "Id,Title,PriorityRank,Year,Body,Item_x0020_Cover,Portfolios/Id,Portfolios/Title,File_x0020_Type,FileLeafRef,FileDirRef,ItemRank,ItemType,Url,Created,Modified,Author/Id,Author/Title,Editor/Id,Editor/Title,EncodedAbsUrl&$expand=Author,Editor,Portfolios";
    if (props.siteName == "Offshore Tasks") {
      props.siteName = "OffShoreTask";
    } else if (
      props?.siteName == "Master Tasks" ||
    ...
    try {
      ...
      await web.lists
        .getById(props?.DocumentsListID)

        .items.select(query)
        .filter(
          `(${props?.siteName}/Id eq ${props?.ID})and(File_x0020_Type ne 'msg')`
        )
        .getAll()
        .then((Data: any[]) => {
          let keydoc: any = [];
          if (Data?.length > 0) {
            Data?.map((item: any, index: any) => {
              if (
                ...
                item?.FileLeafRef != undefined
              ) {
                item.Title = item?.FileLeafRef;
              }
              if (
                item?.Title != undefined &&
                item?.Title.includes(item?.File_x0020_Type)
              ) {
              ...
              if (
                item?.Title != undefined &&
                item?.File_x0020_Type != undefined
              ) {
                item.docType = item?.File_x0020_Type;
              } else if (
                item?.Title != undefined &&
                item?.itemSystemObjectType != 1
              ) {
              ...
              item.CreatedDate = moment(item?.Created).format(
                "'DD/MM/YYYY HH:mm'"
              );
              item.ModifiedDate = moment(item?.ModifiedDate).format(
                "'DD/MM/YYYY HH:mm'"
              );
              if (item.ItemRank === 6) {
                keydoc.push(item);
            ...
            if (
              myContextData2?.FunctionCall != undefined &&
              keydoc?.length > 0
            ) {
              myContextData2?.FunctionCall(keydoc, Data[0]?.FileDirRef, false);
            }
            var releventData = Data?.filter(
              (d) => d.ItemRank != 6 && d.ItemRank != 0
            );
    ...
    <>
                  ...
                  <ul className="alignCenter list-none text-break">
                    ...
                    <li className="pe-1">
                      <a href={item.EncodedAbsUrl}>
                        <span
                          className={`alignIcon svg__iconbox svg__icon--${item?.docType}`}
                          title={item?.File_x0020_Type}
                        ></span>
                      </a>
                    </li>
                    <li className="text-ellips">
                      <InfoToolTip
                        withArrow
                        content={` ${item?.Title}.${item?.docType}`}
                        relationship="label"
                        positioning="below"
                      >
                        <a
                    ...
                    <li className="ml-auto ps-1">
                      <span
                        ...
                        onClick={() => editDocumentsLink(item)}
                      ></span>
                    </li>
                  </ul>
                </div>
            ...
            <div className="card-body p-1">
              <ul className="list-none">
                <li>
                  ...
                  >
                    {" "}
                    <span className="svg__iconbox svg__icon--folder wid30 me-2"></span>{" "}
      ...
      {editdocpanel && (
        <EditDocument
          ...
          ColorCode={myContextData2?.ColorCode}
          AllListId={props.AllListId}
          Context={props.Context}
          editdocpanel={editdocpanel}
          siteName={props?.siteName}
          callbackeditpopup={callbackeditpopup}
        />
      )}
    </>
  );
