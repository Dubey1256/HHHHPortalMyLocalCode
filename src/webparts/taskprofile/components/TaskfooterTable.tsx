...
function Filter({
  ...
  return (
    <input
      style={{ width: "100%" }}
      className="me-1 mb-1 mt-1 on-search-cross form-control "
...
function TasksTable(props: any) {
  ...
  const GetSmartmetadata = async () => {
    ...
    smartmetaDetails = await globalCommon.getData(
      ...
      props?.AllListId?.SmartMetadataListID,
      select
    );
    ...
    smartmetaDetails.forEach((newtest: any) => {
      ...
      if (
        newtest.TaxType == "Sites" &&
        newtest.Item_x005F_x0020_Cover != undefined
      ) {
    ...
    // var filter: any = '';
    if (
      props.props.TaskType != undefined &&
      props.props.TaskType != undefined &&
      props.props.TaskType === "Activities"
    ) {
      ...
      props.props.TaskType != undefined &&
      props.props.TaskType != undefined &&
      props.props.TaskType === "Workstream"
    ) {
  ...
  const loadWSTasks = async (task: any) => {
    ...
    AllWSTasks = await globalCommon.getData(
      props?.AllListId?.siteUrl,
      task.listId,
      select
    );
  ...
  const LoadAllSiteTasks = async (filter: any) => {
    ...
    try {
      ...
      if (AllTasksMatches != undefined && AllTasksMatches.length > 0) {
        ...
        let allParentTasks = $.grep(AllTasks, function (type: any) {
          return (
            type.ParentTask != undefined &&
            type.ParentTask.Id === props.props.Id &&
            type.TaskType != undefined &&
            type.TaskType.Title != "Workstream"
          );
        });
        if (
          props.props.TaskType != undefined &&
          ...
          allworkstreamTasks = $.grep(AllTasks, function (task: any) {
            return (
              task.TaskType != undefined && task.TaskType.Title === "Workstream"
            );
          });
        if (allworkstreamTasks != undefined && allworkstreamTasks.length > 0) {
          allworkstreamTasks.forEach((obj: any) => {
            ...
            obj.childsLength =
              obj.childs != undefined && obj.childs.length > 0
                ? obj.childs.length
                : 0;
            obj.subRowsLength =
              obj.subRows != undefined && obj.subRows.length > 0
                ? obj.subRows.length
                : 0;
          });
        }
        ...
        temp.ItemRank = "";
        temp.DueDate = "";
        if (allworkstreamTasks === undefined) allworkstreamTasks = [];
  ...
  React.useEffect(() => {
    ...
    if (props.props.Portfolio != undefined) GetComponents(props.props);
    // if (props.props.ParentTask != undefined && props.props.ParentTask.Title != undefined)
    //   props.props.ParentIcon = IsUpdated != undefined && IsUpdated == 'Service' ? 'https://hhhhteams.sharepoint.com/sites/HHHH/SP/SiteCollectionImages/ICONS/Service_Icons/icon_Activity.png' : 'https://hhhhteams.sharepoint.com/sites/HHHH/SP/SiteCollectionImages/ICONS/Shareweb/icon_Activity.png';
    // else if (props.props.TaskType != undefined && props.props.TaskType === 'Activities')
    //   props.props.CurrentIcon = IsUpdated != undefined && IsUpdated == 'Service' ? 'https://hhhhteams.sharepoint.com/sites/HHHH/SP/SiteCollectionImages/ICONS/Service_Icons/icon_Activity.png' : 'https://hhhhteams.sharepoint.com/sites/HHHH/SP/SiteCollectionImages/ICONS/Shareweb/icon_Activity.png';
  ...
  const Call = React.useCallback((childItem: any) => {
    ...
    if (childItem != undefined && childItem.data?.ItmesDelete == undefined) {
      ...
      if (
        childItem.data?.editpopup == undefined &&
        childItem.data?.ItmesDelete == undefined
      ) {
          ...
          <>
            ...
            {row?.original?.Short_x0020_Description_x0020_On != null &&
              row?.original?.Short_x0020_Description_x0020_On != "" && (
                <InfoIconsToolTip
                  Discription={row?.original?.Short_x0020_Description_x0020_On}
                  row={row?.original}
                />
              )}
            ...
            {row?.original?.ProjectTitle != (null || undefined) ? (
              <span>
                <a
                  ...
                  target="_blank"
                  className="hreflink serviceColor_Active"
                  href={`${props?.AllListId.siteUrl}/SitePages/Project-Management.aspx?ProjectId=${row?.original?.ProjectId}`}
                >
                  <ReactPopperTooltip
    ...
    if (value != undefined) {
      data?.map((obj: any) => {
        if (value == "Task") {
          ...
          if (
            obj?.TaskType?.Title == value &&
            (TaskLevel < obj.TaskLevel || TaskLevel == obj.TaskLevel)
          ) {
      ...
      .then((res: any) => {
        ...
        array.forEach((obj: any, index: any) => {
          ...
          if (
            ...
            obj.ItemType === backupCheckedList[0]?.ItemType &&
          ...
          if (obj.childs != undefined && obj.childs.length > 0) {
            obj.childs.forEach((sub: any, indexsub: any) => {
              ...
              if (
                ...
                sub.ItemType === backupCheckedList[0]?.ItemType &&
                sub.TaskType?.Title === backupCheckedList[0]?.TaskType?.Title &&
                checkUpdate1 == 1
              ) {
    ...
    await web.lists
      ...
      .then((res: any) => {
        ...
        array.forEach((obj: any, index: any) => {
          obj.isRestructureActive = false;
          if (
            obj.Id === NewArrayBackup[0]?.Id &&
            obj.ItemType === NewArrayBackup[0]?.ItemType &&
            obj.TaskType?.Title === NewArrayBackup[0]?.TaskType?.Title &&
            checkUpdate != 3
          ) {
          ...
          if (obj.childs != undefined && obj.childs.length > 0) {
            obj.childs.forEach((sub: any, indexsub: any) => {
              sub.isRestructureActive = false;
              if (
                sub.Id === NewArrayBackup[0]?.Id &&
                sub.ItemType === NewArrayBackup[0]?.ItemType &&
                sub.TaskType?.Title === NewArrayBackup[0]?.TaskType?.Title &&
                checkUpdate != 3
              ) {
              ...
              if (sub.childs != undefined && sub.childs.length > 0) {
                sub.childs.forEach((newsub: any, lastIndex: any) => {
                  ...
                  if (
                    newsub.Id === NewArrayBackup[0]?.Id &&
                    newsub.ItemType === NewArrayBackup[0]?.ItemType &&
                    newsub.TaskType?.Title ===
                      NewArrayBackup[0]?.TaskType?.Title &&
                    checkUpdate != 3
                  ) {
                  ...
                  if (
                    ...
                    newsub.ItemType === backupCheckedList[0]?.ItemType &&
                    newsub.TaskType?.Title ===
  ...
  const openActivity = () => {
    ...
    if (
      checkData != undefined &&
      checkData != null &&
      checkData?.TaskType != null
    ) {
      if (checkData?.TaskType?.Title == "Workstream") {
        checkData["NoteCall"] = "Task";
        console.log(MeetingItems[MeetingItems.length - 1]);
        if (
          MeetingItems[MeetingItems.length - 1]?.ClientTime?.length > 0 &&
          MeetingItems[MeetingItems?.length - 1].ClientTime != undefined
        ) {
          MeetingItems[MeetingItems.length - 1].ClientTime = JSON.parse(
            MeetingItems[MeetingItems?.length - 1]?.ClientTime
          );
        }
  ...
  const callBackData = React.useCallback((checkData: any) => {
    ...
    if (checkData != undefined) {
      ...
      if (checkData.TaskType != undefined) {
        if (
          checkData.TaskType.Title == "Activities" ||
          checkData.TaskType.Title == "Workstream"
        ) {
          setActivityDisable(false);
          // Arrays.push(itrm)
  ...
  return (
    ...
    >
      ...
      <span>
        <Panel
          headerText={` ${NewArrayBackup[0]?.Title}-Restructuring Tool `}
            ...
            <div className="mt-2">
              ...
              <span>
                <img
                  width={"25px"}
                  height={"25px"}
                  src={OldArrayBackup[0]?.siteIcon}
                />
                {OldArrayBackup[0]?.Title}
              </span>
            </div>
          </div>
        ...
        >
            ...
            <div className="mt-2">
              ...
              <span>
                <input
                  ...
                  value="Task"
                  checked={
                    checkedList[0]?.TaskType?.Title == "Task" ? true : false
                  }
                  onChange={(e) => setRestructure(e)}
                />
          ...
          <footer className="mt-2 text-end">
            <button
              type="button"
              className="btn btn-primary "
              onClick={(e) => UpdateTopTaskRestructure()}
            >
              Save
            </button>
            <button
