...
function TasksTable(props: any) {
    ...
    const OpenModal = (item: any) => {
        ...
        var title = checkedList[0].Item_x0020_Type;
    ...
    const setRestructure = (title: any) => {
    ...
    const UpdateTaskRestructure = async function () {
        ...
        await web.lists.getById(checkedList[0].listId).items.getById(checkedList[0].Id).update({
            ...
            SharewebTaskLevel1No: NewArrayBackup[0].SharewebTaskLevel1No,
            SharewebTaskLevel2No: NewArrayBackup[0].SharewebTaskLevel2No,
        }).then((res: any) => {
    ...
    const UpdateRestructure = async function () {
        let workspacecount = $.grep(data, function (task: any) { return (task.SharewebTaskType != undefined && task.SharewebTaskType.Title === 'Workstream') });
        let taskcount: any = 1;
        taskcount = data.forEach((task) => {
            if (task.Title != undefined && task.Title === 'Taks')
                taskcount = task.childs != undefined ? task.childs.length + 1 : 1;
        })
        ...
        if (ChengedItemTitl != undefined && ChengedItemTitl != "") {
            ...
            await web.lists
                ...
                .update({
                    SharewebTaskTypeId: ChengedItemTitl === 'Task' ? 2 : 3,
                    // SharewebTaskLevel1No: LatestNumber,
                    SharewebTaskLevel2No: ChengedItemTitl === 'Workstream' ? (workspacecount === undefined || workspacecount.length === 0 ? 1 : workspacecount.length + 1) : null,
                    ParentTaskId: props.props.Id,
                                                                                ...
                                                                                <td className="p-0" colSpan={13}>
                                                                                    <table className="table m-0" style={{ width: "100%" }}>
                                                                                        <tr className="for-c02">
                                                                                            ...
                                                                                            <td style={{ width: "6%" }}>
                                                                                                <span className='pe-2'><input type="checkbox" onChange={(e) => onChangeHandler(childitem)} />
                                                                                                    <a className="hreflink" data-toggle="modal">
                ...
                <div>
                    {ResturuningOpen ?
                        <div className='bg-ee p-2 restructurebox'>
                            <div>
                                {NewArrayBackup != undefined && NewArrayBackup.length > 0 ? <span>All below selected items will be added as Task inside  <img className="icon-sites-img me-1 " src={NewArrayBackup[0].SiteIcon}></img> <a data-interception="off" target="_blank" className="hreflink serviceColor_Active"
                            ...
                            <div> {NewArrayBackup != undefined && NewArrayBackup.length > 0 && (NewArrayBackup[0].Id === props.props.Id) ?
                                <div>
                                    <span> {'Select Task Type. :'}<input type="radio" name="fav_language" value="Workstream" checked={ChengedItemTitl === "Workstream" ? true : false} onChange={(e) => setRestructure('Workstream')} /><label className="ms-1"> {'Workstream'} </label></span>
                                    <span> <input type='radio' name="fav_language" value="Task" checked={ChengedItemTitl === "Task" ? true : false} onChange={(e) => setRestructure('Task')} /> <label className="ms-1"> {'Task'} </label> </span>
                                </div>
                                : ''}</div>
                            {NewArrayBackup != undefined && NewArrayBackup.length > 0 && (NewArrayBackup[0].Id !== props.props.Id) ?
                                <div>
