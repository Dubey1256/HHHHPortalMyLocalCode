...
const CreateHHHTask :any= props?.props?.Context ?._pageContext ?._legacyPageContext  ?.tenantDisplayName!=="HHHHTeams"
    ...
    const GetResult = async () => {
        ...
        try {
            ...
            let tempTask = {
                ...
                Approver: taskDetails?.Approver != undefined ? copytaskuser.filter((userData:any) => taskDetails?.Approver?.find((approver:any)=>userData?.AssingedToUser?.Id==approver?.Id )) : "",
            ...
            if(tempTask?.Approver?.length>0){
                ...
                tempTask.checkIsApproval = tempTask?.Approver?.some((checkisApproval:any)=>checkisApproval.AssingedToUser?.Id==copycurrentuser[0].Id)
            }
    ...
    const getsmartmetadataIcon = async () => {
        ...
        try {
            let data = await web.lists
                ...
                .items
                .select('Id', 'Title', 'Item_x0020_Cover', 'TaxType', 'siteName', 'siteUrl', 'Item_x005F_x0020_Cover','Configurations')
                .filter("TaxType eq 'Sites' or TaxType eq 'UxTaskConfiguration'")
                .top(4000)
                .get();
               let  copyUxTaskConfiguration= data?.filter((data2:any)=>data2.TaxType=="UxTaskConfiguration")
               UxTaskConfiguration=      JSON.parse(copyUxTaskConfiguration[0]?. Configurations)         
            smartMetaDataIcon = data?.filter((data2:any)=>data2?.TaxType !="UxTaskConfiguration");
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const CategoryCallBack = React?.useCallback(
        (selectCategoryDataCallBack: any) => {
            let category=""
            let taskcat:any=[]
            if (selectCategoryDataCallBack && selectCategoryDataCallBack?.length > 0) {
                selectCategoryDataCallBack.forEach((item: any, index: any) => {
                    if (index === selectCategoryDataCallBack?.length - 1 || selectCategoryDataCallBack?.length === 1) {
                        category += item.Title;
                    } else {
                        category += item.Title + "; ";
                    }
                        ...
                        <section className='row m-0'>
                            <h2 className="heading d-flex p-0 justify-content-between align-items-center task-title">
                                <span className='alignCenter'>
                                    ...
                                    {state.sendMail ==false && state?.Result?.Approver != undefined && state?.Result?.Approver != ""  && ( state?.Result?.checkIsApproval)&& state?.Result?.Status == "For Approval" &&
                                                    ...
                                                    <dl>
                                                        ...
                                                        <dd className='bg-Ff text-break'>
                                                            <div className='alignCenter justify-content-between'>
                                                                ...
                                                                <span><a className="pancil-icons">
                                                                    <span className="svg__iconbox svg__icon--editBox alignIcon" onClick={()=>setOpenCategories(true)}></span>
                                                                </a></span>
                                                    ...
                                                    <dl>
                                                        ...
                                                        <dd className='bg-Ff'>{state?.Result?.PercentComplete != undefined ? state?.Result?.PercentComplete?.toFixed(0) : 0} <span className='me-2'>%</span> {state?.Result?.Status}<br></br>
                                                            {state?.Result?.ApproverHistory != undefined && state?.Result?.ApproverHistory.length > 1 && state?.Result?.Categories?.includes("Approval") ?
                                                                ...
                                                                : null}
                                                            {state?.Result?.PercentComplete != undefined && state?.Result?.PercentComplete?.toFixed(0) == 70 ? <div className="hover-text">
                                                                <VscIssueReopened
                                    ...
                                    <section>
                                        <div className="col mt-2">
                                            <div className="Taskaddcomment row">
                                                    ...
                                                    <div className="bg-white col-sm-4 mt-2 p-0 boxshadow mb-3">
                                                        ...
                                                        <div className='alignCenter'>
                                                            <div className='alignCenter ml-auto pt-1 gap-1 px-3'>
                                                                ...
                                                                >
                                                                    <span onClick={() => SetOpenComparePopup(checkedImageData?.length <= 1 ?true:false)} className={`svg__iconbox svg__icon--fullScreen ${checkedImageData?.length <= 1 ? 'siteColor' : ""}`}></span>
                                                                </Tooltip>
                                                                <Tooltip
                                                                    withArrow
                                                                    ...
                                                                    positioning="below"
                                                                >
                                                                    <span onClick={() => SetOpenComparePopup(checkedImageData?.length == 2 ?true:false)} className={`svg__iconbox svg__icon--compare2 ${checkedImageData?.length == 2 ? 'siteColor' : ""}`}></span>
                                                                </Tooltip>
                                                                <Tooltip
                                                                    withArrow
                                                                    ...
                                                                    relationship="label" positioning="below"
                                                                >
                                                                    <span onClick={() => SetOpenComparePopup((checkedImageData?.length == 3 || checkedImageData?.length == 4)?true:false)} className={`svg__iconbox svg__icon--compareSeveral ${(checkedImageData?.length == 3 || checkedImageData?.length == 4) ? 'siteColor' : ""}`}></span>
                                                                </Tooltip>
                                                                <Tooltip
                                                                    withArrow
                                                                    content="View All"
                                                                    relationship="label" positioning="below"
                                                                >
                                                                    <span onClick={() => SetOpenComparePopup(checkedImageData?.length > 4? true:false)} className={`svg__iconbox svg__icon--viewAll ${(checkedImageData?.length > 4) ? 'siteColor' : ""}`}></span>
                                                                </Tooltip>
                                                ...
                                                {UxTaskConfiguration?.some((uxdata:any)=>state?.Result?.Categories?.includes(uxdata?.Title)) ? <Uxdescriptions Item={state?.Result} currentUser={currentUser} siteUrl={propsValue?.siteUrl} listName={listName} itemID={itemID} ApprovalStatus={ApprovalStatus} AllListId={AllListId} taskUsers={taskUsers} call={GetResult} userDisplayName={props?.userDisplayName} />
                                        ...
                                        {backGroundComment && <div className="col mt-2">
                                            <div className="Taskaddcomment row">
                                                ...
                                                {state?.Result?.OffshoreComments != null && state?.Result?.OffshoreComments != undefined && state?.Result?.OffshoreComments?.length > 0 && state?.Result?.OffshoreComments?.some((item: any) => item?.Type !== "EODReport")  && <div className="col-sm-8 pe-0 mt-2">
                                                    <fieldset className='border p-1'>
                                                        ...
                                                        {state?.Result?.OffshoreComments != null && state?.Result?.OffshoreComments?.length > 0 && state?.Result?.OffshoreComments?.map((item: any, index: any) => {
                                                            return item?.Type !== "EODReport" && <div>
                                                                <span className='round px-1'>
