...
const CopyTaskProfile = (props: any) => {
  ...
  const CreateHHHTask: any =
    props?.props?.Context?._pageContext?._legacyPageContext
      ?.tenantDisplayName !== "HHHHTeams";
  const [TagConceptPaper, setTagConceptPaper] = useState([]);
  const [isopenProjectpopup, setisopenProjectpopup] = useState(false);
  const [isopencomonentservicepopup, setisopencomonentservicepopup] = useState(
    false
  );
  ...
  const [openCategories, setOpenCategories]: any = useState(false);
  ...
  useEffect(() => {
    const capitalizeFLetter = (site: any) =>
      site[0].toUpperCase() + site.slice(1);
    setSite(capitalizeFLetter(siteParam));

    setOldTaskLink(
      `${propsValue?.siteUrl}/SitePages/Task-Profile-Old.aspx?taskId=${taskId}&Site=${siteParam}`
    );
  ...
  const GetAllComponentAndServiceData = async (ComponentType: any) => {
    ...
    if (CallBackData?.AllData && CallBackData?.AllData.length > 0) {
      masterTaskData = masterTaskData.concat([
        ...CallBackData?.FlatProjectData,
        ...CallBackData?.AllData,
      ]);
      masterForHierarchy = masterForHierarchy.concat([
        ...CallBackData?.FlatProjectData,
        ...CallBackData?.AllData,
      ]);
  ...
  const GetResult = async () => {
    ...
    try {
      isShowTimeEntry =
        propsValue?.TimeEntry !== "" ? JSON.parse(propsValue?.TimeEntry) : "";
      setisShowSiteCompostion(
        propsValue?.SiteCompostion !== ""
          ? JSON.parse(propsValue?.SiteCompostion)
          : ""
      );
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      let category = "";
      if (
        taskDetails["TaskCategories"] &&
        taskDetails["TaskCategories"].length > 0
      ) {
        taskDetails["TaskCategories"].forEach((item: any, index: any) => {
          if (
            index === taskDetails["TaskCategories"].length - 1 ||
            taskDetails["TaskCategories"].length === 1
          ) {
            category += item.Title;
          } else {
            category += item.Title + "; ";
      ...
      await GetSmartMetaData(
        taskDetails?.ClientCategory,
        taskDetails?.Sitestagging
      );
      ...
      if (
        taskDetails["Comments"] != null &&
        taskDetails["Comments"] != undefined
      ) {
        try {
      ...
      if (taskDetails["EstimatedTimeDescription"]?.length > 0) {
        tempEstimatedArrayData = JSON.parse(
          taskDetails["EstimatedTimeDescription"]
        );
        if (tempEstimatedArrayData?.length > 0) {
          tempEstimatedArrayData?.map((TimeDetails: any) => {
            TotalEstimatedTime =
              TotalEstimatedTime + Number(TimeDetails.EstimatedTime);
          });
        }
      } else {
      ...
      if (taskDetails["Title"].length > maxTitleLength) {
        truncatedTitle =
          taskDetails["Title"].substring(0, maxTitleLength - 3) + "...";
      }

      let portfolio: any = [];
      if (taskDetails?.Portfolio != undefined) {
        portfolio = masterTaskData.filter(
          (item: any) => item.Id == taskDetails?.Portfolio?.Id
        );
        if (
          portfolio?.length > 0 &&
          portfolio[0]?.PortfolioType?.Color != undefined
        ) {
          document?.documentElement?.style?.setProperty(
            "--SiteBlue",
            portfolio[0]?.PortfolioType?.Color
          );
        }
      ...
      if (taskDetails?.Project != undefined) {
        ProjectData = masterTaskData?.find(
          (items: any) => items?.Id == taskDetails?.Project?.Id
        );
      }
      let feedBackData: any = JSON.parse(taskDetails["FeedBack"]);
      console.log(masterTaskData);
      let WorkingAction =
        taskDetails["WorkingAction"] != null
          ? JSON.parse(taskDetails["WorkingAction"])
          : [];
      let Bottleneck: any = [];
      let Attention: any = [];
      let Phone: any = [];
        ...
        WorkingAction?.map((Action: any) => {
          ...
          if (Action?.Title == "WorkingDetails") {
            ...
            Action?.InformationData?.map((isworkingToday: any) => {
              if (
                isworkingToday?.WorkingDate == currentDate &&
                isworkingToday?.WorkingMember?.length > 0
              ) {
                taskDetails["IsTodaysTask"] = true;
                let workingTodayUser: any = [];
                isworkingToday?.WorkingMember?.map((itm: any) => {
                  let filterworkingTodayUser = copytaskuser.filter(
                    (x: any) => x.AssingedToUser?.Id == itm?.Id
                  );
                  workingTodayUser = [
                    ...workingTodayUser,
                    filterworkingTodayUser[0],
                  ];
                });
      ...
      let tempTask = {
        ...
        taskPriorityOnHover:
          taskDetails?.PriorityRank != undefined
            ? taskDetails?.PriorityRank
            : undefined,
        showFormulaOnHover:
          taskDetails?.showFormulaOnHover != undefined
            ? taskDetails?.showFormulaOnHover
            : undefined,
        Approvee:
          taskDetails?.Approvee != undefined
            ? taskUsers.find(
                (userData: any) =>
                  userData?.AssingedToUser?.Id == taskDetails?.Approvee?.Id
              )
            : undefined,
        ...
        Sitestagging:
          taskDetails["Sitestagging"] != null
            ? JSON.parse(taskDetails["Sitestagging"])
            : [],
        ApproverHistory:
          taskDetails["ApproverHistory"] != null
            ? JSON.parse(taskDetails["ApproverHistory"])
            : "",
        OffshoreComments:
          OffshoreComments.length > 0 ? OffshoreComments.reverse() : null,
        OffshoreImageUrl:
          taskDetails["OffshoreImageUrl"] != null &&
          JSON.parse(taskDetails["OffshoreImageUrl"]),
        ...
        StartDate:
          taskDetails["StartDate"] != null
            ...
            : "",
        CompletedDate:
          taskDetails["CompletedDate"] != null
            ? moment(taskDetails["CompletedDate"])?.format("DD/MM/YYYY")
            : "",
        TeamLeader:
          taskDetails["ResponsibleTeam"] != null
            ? taskDetails["ResponsibleTeam"]
            : null,
        ResponsibleTeam:
          taskDetails["ResponsibleTeam"] != null
            ? taskDetails["ResponsibleTeam"]
            : null,
        TeamMembers:
          taskDetails.TeamMembers != null ? taskDetails.TeamMembers : null,
        AssignedTo:
          taskDetails["AssignedTo"] != null ? taskDetails["AssignedTo"] : null,
        ...
        FeedBackArray:
          feedBackData != undefined && feedBackData?.length > 0
            ? feedBackData[0]?.FeedBackDescriptions
            : [],
        TaskType:
          taskDetails["TaskType"] != null ? taskDetails["TaskType"] : "",
        TaskTypeTitle:
          taskDetails["TaskType"] != null ? taskDetails["TaskType"]?.Title : "",
        ...
        Approver:
          taskDetails?.Approver != undefined
            ? copytaskuser.filter((userData: any) =>
                taskDetails?.Approver?.find(
                  (approver: any) =>
                    userData?.AssingedToUser?.Id == approver?.Id
                )
              )
            : "",
      ...
      if (tempTask?.Approver?.length > 0) {
        tempTask.checkIsApproval = tempTask?.Approver?.some(
          (checkisApproval: any) =>
            checkisApproval.AssingedToUser?.Id == copycurrentuser[0].Id
        );
      }
      ...
      setState((prevState: any) => ({
        ...
        Result: tempTask,
        FileDirRef:
          taskDetails.AttachmentFiles.length > 0
            ? taskDetails.AttachmentFiles[0].ServerRelativeUrl
            : "",
  ...
  const getsmartmetadataIcon = async () => {
    ...
    try {
      let data = await web.lists
        ...
        .filter("TaxType eq 'Sites' or TaxType eq 'UxTaskConfiguration'")
        .top(4000)
        .get();
      let copyUxTaskConfiguration = data?.filter(
        (data2: any) => data2.TaxType == "UxTaskConfiguration"
      );
      UxTaskConfiguration = JSON.parse(
        copyUxTaskConfiguration[0]?.Configurations
      );
      smartMetaDataIcon = data?.filter(
        (data2: any) => data2?.TaxType != "UxTaskConfiguration"
      );
    } catch (error) {
      console.log(error);
  ...
  const GetTaskUsers = async (taskDetails: any) => {
    ...
    taskUsers?.map((item: any) => {
      if (
        propsValue?.Context?.pageContext?._legacyPageContext?.userId ===
          item?.AssingedToUser?.Id &&
        item?.Company === "HHHH"
      ) {
  ...
  const GetSmartMetaData = async (ClientCategory: any, Sitestagging: any) => {
    ...
    if (
      ClientTimeArray != undefined &&
      ClientTimeArray.length > 0 &&
      array2?.length > 0
    ) {
      ...
      ClientTimeArray?.map((item: any) => {
        item.SiteImages = GetSiteIcon(
          item?.SiteName != undefined ? item?.SiteName : item?.Title
        );
      });
    }
  };
      ...
      if (listName != undefined) {
        ...
        smartMetaDataIcon?.map((icondata: any) => {
          if (icondata.Title != undefined) {
            if (
              icondata.Title.toLowerCase() == listName?.toLowerCase() &&
              icondata.Item_x0020_Cover != undefined
            ) {
              siteicon = icondata.Item_x0020_Cover.Url;
            }
            if (
              icondata.Title.toLowerCase() == listName?.toLowerCase() &&
              icondata.Item_x005F_x0020_Cover != undefined
            ) {
  ...
  const updateResult = async (tempTask: any) => {
    ...
    if (tempTask.Portfolio !== undefined) {
      let AllItems: any = [];
      let breadCrumData1WithSubRow = await globalCommon?.getBreadCrumbHierarchyAllData(
        tempTask,
        AllListId,
        AllItems
      );
    ...
    if (username != undefined) {
      ...
      if (senderObject?.length > 0) {
        userDeatails.push({
          ...
          userImage:
            senderObject[0]?.Item_x0020_Cover != null
              ? senderObject[0]?.Item_x0020_Cover.Url
              : "",
        });
      }
      if (senderObject.length == 0) {
  ...
  const sendEmail = (item: any) => {
    ...
    if (item == "Approved") {
      ...
      TeamMembers.push(state?.Result?.TeamMembers[0]?.Id);
      TeamMembers.push(
        state?.Result?.Approvee != undefined
          ? state?.Result?.Approvee?.AssingedToUser?.Id
          : state?.Result?.Author[0]?.Id
      );
      let changeData: any = {
        TeamMembers: TeamMembers,
        AssignedTo: [
  ...
  const ChangeApprovalMember = (changeData: any) => {
    return new Promise<void>((resolve, reject) => {
      ...
      web.lists
        .getByTitle(state?.Result?.listName)

        .items.getById(state?.Result?.Id)
        .update({
          TeamMembersId: {
    ...
    if (txtComment != "") {
      ...
      if (state?.isEditReplyModalOpen) {
        ...
        if (EditReplyData?.isSubtextComment) {
          let feedback =
            state?.Result?.FeedBack[0]?.FeedBackDescriptions[
              EditReplyData?.parentIndexOpeneditModal
            ].Subtext[EditReplyData?.indexOfSubtext].Comments[
              EditReplyData?.indexOfUpdateElement
            ].ReplyMessages[EditReplyData?.replyIndex];
          feedback.Title = state?.CommenttoUpdate;
        } else {
          let feedback =
            state?.Result?.FeedBack[0]?.FeedBackDescriptions[
              EditReplyData?.parentIndexOpeneditModal
            ].Comments[EditReplyData?.indexOfUpdateElement].ReplyMessages[
              EditReplyData?.replyIndex
            ];
          feedback.Title = state?.CommenttoUpdate;
        }
      } else {
        if (state?.updateCommentText?.data?.isApprovalComment) {
          temp.isApprovalComment =
            state?.updateCommentText?.data?.isApprovalComment;
        ...
        if (state?.updateCommentText?.isSubtextComment) {
          state.Result.FeedBack[0].FeedBackDescriptions[
            state?.updateCommentText?.parentIndexOpeneditModal
          ].Subtext[state.updateCommentText["indexOfSubtext"]]["Comments"][
            state.updateCommentText["indexOfUpdateElement"]
          ].Title = temp.Title;
        } else {
          state.Result.FeedBack[0].FeedBackDescriptions[
            state?.updateCommentText?.parentIndexOpeneditModal
  ...
  const SaveReplyMessageFunction = () => {
    ...
    if (txtComment != "") {
      ...
      if (index.length == 2) {
        ...
        let feedback =
          state?.Result?.FeedBack[0]?.FeedBackDescriptions[parentIndex]
      ...
      if (index.length == 3) {
        ...
        let feedback =
          state?.Result?.FeedBack[0]?.FeedBackDescriptions[parentIndex].Subtext[
            subcomentIndex
          ].Comments[commentIndex];
  ...
  const onRenderCustomHeadereditcomment = () => {
    return (
      <>
        <div className="subheading">Update Comment</div>
  ...
  const loadTaggedConceptPaperDocument = async (Documents: any) => {
    ...
    try {
      let query =
        "Id,Title,PriorityRank,DocumentType,Year,Body,Item_x0020_Cover,Portfolios/Id,Portfolios/Title,File_x0020_Type,FileLeafRef,FileDirRef,ItemRank,ItemType,Url,Created,Modified,Author/Id,Author/Title,Editor/Id,Editor/Title,EncodedAbsUrl&$expand=Author,Editor,Portfolios";
      await web.lists
        .getById(AllListId?.DocumentsListID)
        .items.select(query)
        .filter(`(Portfolios/Id eq ${Documents?.ID})`)
        .getAll()
        .then((Data: any[]) => {
          var tagdoc: any = Data.filter(
            (item: any) => item.DocumentType === "Concept Paper"
          );
  ...
  const ComponentServicePopupCallBack = async (
    DataItem: any,
    Type: any,
    functionType: any
  ) => {
      ...
      if (isopencomonentservicepopup) {
          ...
          if (state?.Result?.siteType?.toLowerCase() == "shareweb") {
            ...
            sitetag?.map((sitecomp: any) => {
              if (
                ...
                let ClientCategory = cctag?.filter(
                  (data: any) => data?.siteName == sitecomp.Title
                );
                if (ClientCategory.length > 0) {
        ...
        DataItem?.map((portfolio: any) => {
          portfolio?.ClientCategory?.map((cc: any) => {
            if (cc.Id != undefined) {
              let foundCC = AllClientCategories?.find(
                (allCC: any) => allCC?.Id == cc.Id
              );
              if (state?.Result?.siteType?.toLowerCase() == "shareweb") {
                selectedCC.push(cc.Id);
                cctag.push(foundCC);
              } else if (
                state?.Result?.siteType?.toLowerCase() ==
                foundCC?.siteName?.toLowerCase()
              ) {
                selectedCC.push(cc.Id);
                cctag.push(foundCC);
              }
        ...
        if (
          DataItem[0]?.Item_x0020_Type == "Project" ||
          DataItem[0]?.Item_x0020_Type == "Sprint"
        ) {
          let resultData = state.Result;
          ...
          setState((prevState: any) => ({
            ...
            Result: {
              ...prevState.Result,
              ["SmartPriority"]: globalCommon?.calculateSmartPriority(
                resultData
              ),
            },
          }));
          ...
          if (state.Result?.TaskType?.Title != "Task") {
            await globalCommon?.AwtGroupingAndUpdatePrarticularColumn(
              state.Result,
              allDataOfTask,
              dataUpdate
            );
  ...
  const SendRemindernotifications = (InfoData: any, ActionType: any) => {
    if (InfoData?.NotificationSend == true) {
      ...
      alert(
        `This user has not been tagged as a ${ActionType} yet, so you cannot send a reminder now.`
      );
    }
  };
  ...
  const CheckImageData = (value: boolean, imgeData: any) => {
    ...
    if (value) {
      ...
      copyCheckedImageData = copyCheckedImageData?.filter(
        (data: any) => data.ImageName != imgeData.ImageName
      );
    }

    SetCheckedImageData([...copyCheckedImageData]);
  };
  const CategoryCallBack = React?.useCallback(
    (selectCategoryDataCallBack: any) => {
      let category = "";
      let taskcat: any = [];
      if (
        selectCategoryDataCallBack &&
        selectCategoryDataCallBack?.length > 0
      ) {
        selectCategoryDataCallBack.forEach((item: any, index: any) => {
          if (
            index === selectCategoryDataCallBack?.length - 1 ||
            selectCategoryDataCallBack?.length === 1
          ) {
            category += item.Title;
          } else {
            category += item.Title + "; ";
          ...
          taskcat.push(item?.Id);
        });
        setState((prevState: any) => ({
          ...prevState,
          Result: {
            ...prevState.Result,
            ["TaskCategories"]: selectCategoryDataCallBack,
            ["Categories"]: category,
          },
        }));
        ...
        updateProjectComponentServices(updateObject);
      }
    },
    []
  );
      ...
      >
        <div className="taskprofilesection">
          <section className="ContentSection">
            {" "}
            {state.breadCrumData != undefined && (
              <div className="row m-0">
                <div className="col-sm-12 p-0 ">
                  <ul className="webbreadcrumbs ">
                    {state?.Result?.Portfolio == undefined &&
                    state.breadCrumData?.length == 0 &&
                    state?.Result?.Title != undefined ? (
                      <>
                        <li>
                          <a
                            target="_blank"
                            data-interception="off"
                            href={`${state?.Result?.siteUrl}/SitePages/Dashboard.aspx`}
                          >
                            {" "}
                            <span>Dashboard</span>{" "}
                          </a>{" "}
                      ...
                      <>
                            ...
                            <li>
                              {state?.Result?.Portfolio != null && (
                                <a
                                  ...
                                  style={{
                                    color:
                                      state?.Result?.Portfolio?.PortfolioType
                                        ?.Color,
                                  }}
                                  target="_blank"
                                  data-interception="off"
                                  href={`${state?.Result?.siteUrl}/SitePages/Team-Portfolio.aspx`}
                                >
                                  Team portfolio
                                </a>
                              )}
                        ...
                        {state.breadCrumData?.map(
                          (breadcrumbitem: any, index: any) => {
                            return (
                              <>
                                {breadcrumbitem?.siteType == "Master Tasks" && (
                                  <li>
                                    <a
                                      style={{
                                        color:
                                          breadcrumbitem?.PortfolioType?.Color,
                                      }}
                                      target="_blank"
                                      data-interception="off"
                                      href={`${state?.Result?.siteUrl}/SitePages/Portfolio-Profile.aspx?taskId=${breadcrumbitem?.Id}`}
                                    >
                                ...
                                {breadcrumbitem?.siteType !==
                                  "Master Tasks" && (
                                  <li>
                                    <a
                                      target="_blank"
                                      data-interception="off"
                                      href={`${state?.Result?.siteUrl}/SitePages/Task-Profile.aspx?taskId=${breadcrumbitem?.Id}&Site=${breadcrumbitem?.siteType} `}
                                    >
                                      {breadcrumbitem?.Title}
                                    </a>
                                    {breadcrumbitem?.Id != itemID && (
                                      <span>
                                        {" "}
                                        <SlArrowRight />
                                      </span>
            ...
            <section className="row m-0">
              <h2 className="heading d-flex p-0 justify-content-between align-items-center task-title">
                <span className="alignCenter">
                  {state?.Result?.SiteIcon != "" && (
                    <img
                      className="imgWid29 pe-1 "
                      title={state?.Result?.siteType}
                      src={state?.Result?.SiteIcon}
                    />
                  )}
                  {state?.Result?.SiteIcon === "" && (
                    <img className="imgWid29 pe-1 " src="" />
                  )}
                  <span
                    className="popover__wrapper ms-1"
                    data-bs-toggle="tooltip"
                  ...
                  state?.Result?.Approver != undefined &&
                  state?.Result?.Approver != "" &&
                  state?.Result?.checkIsApproval &&
                  state?.Result?.Status == "For Approval" &&
                  state?.Result?.PercentComplete == 1 ? (
                    <span>
                      <button
                        onClick={() => sendEmail("Approved")}
                        className="btn btn-success ms-3 mx-2"
                      >
                        Approve
                      </button>
                      <span>
            ...
            <section>
              <div className="row m-0">
                  ...
                  <div className="team_member row">
                      ...
                      <div className="bg-Ff p-2 boxshadow  rounded-1 row">
                        <div className="col-md-6 p-0">
                          <dl>
                            ...
                            <dd className="bg-Ff position-relative">
                              <ReactPopperTooltipSingleLevel
                                CMSToolId={state?.Result?.TaskId}
                                row={state.Result}
                                singleLevel={true}
                                masterTaskData={masterForHierarchy}
                                AllSitesTaskData={allDataOfTask}
                                AllListId={AllListId}
                              />
                            </dd>
                          </dl>
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              {state?.Result?.StartDate != undefined
                                ? state?.Result?.StartDate
                                : ""}
                            </dd>
                          </dl>
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              {" "}
                              {state?.Result?.CompletedDate != undefined
                                ? state?.Result?.CompletedDate
                                : ""}
                            </dd>
                          </dl>
                          <dl>
                            <dt className="bg-Fa" title="Task Id">
                              Categories
                            </dt>

                            <dd className="bg-Ff text-break">
                              <div className="alignCenter justify-content-between">
                                <span>{state?.Result?.Categories}</span>
                                <span>
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              <div className="boldClable">
                                <span
                                  className={
                                    state?.Result?.SmartPriority != undefined
                                      ? "hover-text hreflink m-0 r sxsvc"
                                      : "hover-text hreflink m-0 cssc"
                                  }
                                >
                                  <>
                                    {state?.Result?.SmartPriority != undefined
                                      ? state?.Result?.SmartPriority
                                      : 0}
                                  </>
                                  <span className="tooltip-text pop-right">
                                    {state?.Result?.showFormulaOnHover !=
                                    undefined ? (
                                      <SmartPriorityHover
                                        editValue={state.Result}
                                      />
                                    ) : (
                                      ""
                                    )}
                          ...
                          {isShowTimeEntry && (
                            <dl>
                              ...
                              <dd className="bg-Ff">
                                <span className="me-1 alignCenter  pull-left">
                                  {" "}
                                  {state.smarttimefunction ? (
                                    <SmartTimeTotal
                                      AllListId={AllListId}
                                      callbackTotalTime={(data: any) =>
                                        callbackTotalTime(data)
                                      }
                                      props={state.Result}
                                      Context={propsValue.Context}
                                      allTaskUsers={taskUsers}
                                    />
                                  ) : null}
                                </span>
                              </dd>
                        ...
                        <div className="col-md-6 p-0">
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              {state?.Result?.PercentComplete != undefined
                                ? state?.Result?.PercentComplete?.toFixed(0)
                                : 0}{" "}
                              <span className="me-2">%</span>{" "}
                              {state?.Result?.Status}
                              <br></br>
                              {state?.Result?.ApproverHistory != undefined &&
                              state?.Result?.ApproverHistory.length > 1 &&
                              state?.Result?.Categories?.includes(
                                "Approval"
                              ) ? (
                                <span style={{ fontSize: "smaller" }}>
                                  ...
                                  <img
                                    ...
                                    title={
                                      state?.Result?.ApproverHistory[
                                        state.Result?.ApproverHistory.length - 2
                                      ]?.ApproverName
                                    }
                                    src={
                                      state.Result?.ApproverHistory[
                                        state.Result?.ApproverHistory?.length -
                                          2
                                      ]?.ApproverImage != null
                                        ? state.Result.ApproverHistory[
                                            state.Result.ApproverHistory
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff position-relative">
                              {state?.Result?.workingTodayUser != undefined &&
                                state?.Result?.workingTodayUser?.map(
                                  (user: any) => {
                                    return (
                                      <>
                                        ...
                                        undefined ? (
                                          ...
                                          <span
                                            ...
                                            onClick={() =>
                                              globalCommon?.openUsersDashboard(
                                                AllListId?.siteUrl,
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              ...
                              state?.Result?.Categories?.indexOf("On-Hold") >=
                                0 ? (
                                <div className="hover-text">
                                  <IoHandRightOutline
                                    onMouseEnter={showOnHoldReason}
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              {state?.Result?.Phone?.length > 0 &&
                                state?.Result?.Phone?.map((PhoneData: any) => {
                                  return (
                                    <div className="align-content-center alignCenter justify-content-between py-1">
                                      <div className="">
                                        {PhoneData.TaggedUsers.userImage !=
                                          undefined &&
                                        PhoneData.TaggedUsers.userImage.length >
                                          0 ? (
                                          <img
                                            className="ProirityAssignedUserPhoto m-0"
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff">
                              {state?.Result?.Approval?.length > 0 &&
                                state?.Result?.Approval?.map(
                                  (ApprovalData: any) => {
                                    return (
                                      <div className="align-content-center alignCenter justify-content-between py-1">
                                        <div className="">
                          ...
                          <dl>
                            ...
                            <dd className="bg-Ff alignCenter">
                              {state?.Result?.Created != undefined &&
                              state?.Result?.Created != null
                                ? moment(state?.Result?.Created).format(
                                    "DD/MMM/YYYY"
                                  )
                                : ""}
                              {state?.Result?.Author != null &&
                                state?.Result?.Author.length > 0 && (
                                  <a
                                    title={state?.Result?.Author[0].Title}
                                    className="alignCenter ms-1"
                                  >
                                    {state?.Result?.Author[0].userImage !==
                                      "" && (
                                      <img
                                        className="workmember hreflink "
                                        src={state?.Result?.Author[0].userImage}
                                        onClick={() =>
                                          globalCommon?.openUsersDashboard(
                                            AllListId?.siteUrl,
                                            state?.Result?.Author[0]?.Id
                                    ...
                                    {state?.Result?.Author[0].userImage ===
                                      ...
                                      <span
                                        title={`${
                                          state?.Result?.Author != undefined
                                            ? state?.Result?.Author[0].Title
                                            : "Default user icons "
                                        }`}
                                        className="alignIcon hreflink  svg__iconbox svg__icon--defaultUser"
                                        onClick={() =>
                        ...
                        <div className="col-12 p-0">
                          <dl>
                            ...
                            <dt
                              className="bg-Ff p-2 text-break "
                              style={{ width: "80%" }}
                            >
                              {state?.Result?.component_url != null && (
                                <a
                                  target="_blank"
                                  data-interception="off"
                                  href={state?.Result?.component_url.Url}
                                >
                    ...
                    <div className="col-md-4 pe-0 Site_Compositionbox">
                      <div className="bg-Ff p-2 rounded-1 boxshadow h-100 ">
                        <dl>
                          ...
                          <dd className="bg-Ff full-width columnFixedTitle pe-0">
                            {TagConceptPaper?.length > 0 && (
                              <a
                                href={`${TagConceptPaper[0].EncodedAbsUrl}?web=1`}
                                rel="noopener noreferrer"
                                target="_blank"
                                data-interception="off"
                              >
                                <span
                                  className={`alignIcon svg__iconbox svg__icon--${TagConceptPaper[0]?.File_x0020_Type}`}
                                  title={TagConceptPaper[0]?.File_x0020_Type}
                                ></span>
                              </a>
                            )
                            ...
                            {state?.Result?.Portfolio != null && (
                              <a
                                className="hreflink text-content w-100"
                                target="_blank"
                                data-interception="off"
                                href={`${state?.Result?.siteUrl}/SitePages/Portfolio-Profile.aspx?taskId=${state?.Result?.Portfolio.Id}`}
                              >
                          ...
                          <dd className="bg-Ff full-width columnFixedTitle pe-0">
                            {ProjectData?.Title != undefined ? (
                              <a
                                ...
                                target="_blank"
                                data-interception="off"
                                href={`${state?.Result?.siteUrl}/SitePages/PX-Profile.aspx?ProjectId=${ProjectData?.Id}`}
                              >
                                <ReactPopperTooltipSingleLevel
                                  CMSToolId={`${ProjectData?.PortfolioStructureID} - ${ProjectData?.Title}`}
                                  row={ProjectData}
                                  singleLevel={true}
                                ...
                                <div className="dropdown">
                                  <a className="sitebutton bg-fxdark d-flex">
                                    ...
                                    <div className="d-flex justify-content-between full-width">
                                      ...
                                      <p
                                        className="input-group-text mb-0 pb-0"
                                        title="Edit Site Composition"
                                        onClick={() =>
                                          setState({
                                            ...state,
                                            EditSiteCompositionStatus: true,
                                          })
                                        }
                                      >
                                  ...
                                  <div
                                    className="spxdropdown-menu"
                                    style={{
                                      display: showComposition
                                        ? "block"
                                        : "none",
                                    }}
                                  >
                                          ...
                                          return (
                                            <li className="Sitelist">
                                              <span>
                                                <img
                                                  style={{ width: "22px" }}
                                                  title={cltime?.SiteName}
                                                  src={cltime?.SiteImages}
                                                />
                                              </span>
                                              ...
                                              {cltime.ClientCategory !=
                                                undefined &&
                                              cltime.ClientCategory.length > 0
                                                ? cltime.ClientCategory?.map(
                                                    (clientcat: any) => {
                                                      return (
                                                        <span>
                                                          {clientcat.Title}
                        ...
                        {state?.Result?.EstimatedTimeDescriptionArray?.length >
                          0 && (
                          <dl className="Sitecomposition my-2">
                            <div className="dropdown">
                              <a className="sitebutton bg-fxdark d-flex">
                                <span
                                  className="arrowicons"
                                  onClick={() => showhideEstimatedTime()}
                                >
                                  {state?.ShowEstimatedTimeDescription ? (
                                    <SlArrowDown />
                                  ) : (
                                    <SlArrowRight />
                                  )}
                              ...
                              <div
                                className="spxdropdown-menu"
                                style={{
                                  display: state?.ShowEstimatedTimeDescription
                                    ? "block"
                                    : "none",
                                }}
                              >
                                ...
                                >
                                  {state?.Result
                                    ?.EstimatedTimeDescriptionArray != null &&
                                  state?.Result?.EstimatedTimeDescriptionArray
                                    ?.length > 0 ? (
                                    <div>
                                      {state?.Result?.EstimatedTimeDescriptionArray?.map(
                                        (
                                          EstimatedTimeData: any,
                                          Index: any
                                        ) => {
                                          return (
                                            <div
                                              className={
                                                state?.Result
                                                  ?.EstimatedTimeDescriptionArray
                                                  ?.length ==
                                                Index + 1
                                                  ? "align-content-center alignCenter justify-content-between p-1 px-2"
                                                  : "align-content-center justify-content-between border-bottom alignCenter p-1 px-2"
                                              }
                                            >
                                              <div className="alignCenter">
                                                <span className="me-2">
                                                ...
                                                <img
                                                  className="ProirityAssignedUserPhoto m-0 mx-2 hreflink "
                                                  onClick={() =>
                                                    globalCommon?.openUsersDashboard(
                                                      AllListId?.siteUrl,
                                                      undefined,
                                                      EstimatedTimeData?.UserName,
                                                      taskUsers
                                                    )
                                                  }
                                                  title={
                              ...
                              <div
                                className="spxdropdown-menu ps-2 py-1 "
                                style={{ zIndex: 0 }}
                              >
                                <span>Total Estimated Time : </span>
                                <span className="mx-1">
                                  {state?.Result?.TotalEstimatedTime > 1
                                    ? state?.Result?.TotalEstimatedTime +
                                      " hours"
                                    : state?.Result?.TotalEstimatedTime +
                                      " hour"}{" "}
                                </span>
                        ...
                        {CreateHHHTask && (
                          <span>
                            <a
                              href={`https://hhhhteams.sharepoint.com/sites/HHHH/SP/SitePages/CreateTask.aspx?TaskId=${state?.Result?.Id}&Title=${state?.Result?.Title}&DueDate=${state?.Result?.DueDate}&Priority=${state?.Result?.Priority}&CreatedBy=${state?.Result?.Author?.[0]?.Title}&CreatedDate=${state?.Result?.Created}&ExternalSite=1&TaskSite=${listName}&siteAbsoluteUrl=${AllListId?.siteUrl}&TaskUrl=${state?.Result?.component_url?.Url}`}
                              target="_blank"
                              data-interception="off"
                            >
                              Create HHHH Task
                  ...
                  <section>
                    <div className="col mt-2">
                      <div className="Taskaddcomment row">
                        {state?.Result?.BasicImageInfo != null &&
                          state?.Result?.BasicImageInfo?.length > 0 && (
                            <div className="bg-white col-sm-4 mt-2 p-0 boxshadow mb-3">
                              <label className="form-label full-width fw-semibold titleheading">
                                Images
                              </label>
                              <div className="alignCenter">
                                <div className="alignCenter ml-auto pt-1 gap-1 px-3">
                                  ...
                                  >
                                    <span
                                      onClick={() =>
                                        SetOpenComparePopup(
                                          checkedImageData?.length <= 1
                                            ? true
                                            : false
                                        )
                                      }
                                      className={`svg__iconbox svg__icon--fullScreen ${
                                        checkedImageData?.length <= 1
                                          ? "siteColor"
                                          : ""
                                      }`}
                                  ...
                                  >
                                    <span
                                      onClick={() =>
                                        SetOpenComparePopup(
                                          checkedImageData?.length == 2
                                            ? true
                                            : false
                                        )
                                      }
                                      className={`svg__iconbox svg__icon--compare2 ${
                                        checkedImageData?.length == 2
                                          ? "siteColor"
                                          : ""
                                      }`}
                                  ...
                                  >
                                    <span
                                      onClick={() =>
                                        SetOpenComparePopup(
                                          checkedImageData?.length == 3 ||
                                            checkedImageData?.length == 4
                                            ? true
                                            : false
                                        )
                                      }
                                      className={`svg__iconbox svg__icon--compareSeveral ${
                                        checkedImageData?.length == 3 ||
                                        checkedImageData?.length == 4
                                          ? "siteColor"
                                          : ""
                                  ...
                                  >
                                    <span
                                      ...
                                      className={`svg__iconbox svg__icon--viewAll ${
                                        checkedImageData?.length > 4
                                          ? "siteColor"
                                          : ""
                                      }`}
                              ...
                              {state?.Result?.BasicImageInfo != null &&
                                state?.Result?.BasicImageInfo?.map(
                                  (imgData: any, i: any) => {
                                    return (
                                      <div className="taskimage  mb-3">
                        ...
                        {UxTaskConfiguration?.some((uxdata: any) =>
                          state?.Result?.Categories?.includes(uxdata?.Title)
                        ) ? (
                          <Uxdescriptions
                            Item={state?.Result}
                            currentUser={currentUser}
                    ...
                    {backGroundComment && (
                      <div className="col mt-2">
                        <div className="Taskaddcomment row">
                          {state?.Result?.OffshoreImageUrl != null &&
                            state?.Result?.OffshoreImageUrl.length > 0 && (
                              <div className="bg-white col-sm-4 mt-2 p-0 boxshadow">
                                {state?.Result?.OffshoreImageUrl != null &&
                                  state?.Result?.OffshoreImageUrl?.map(
                                    (imgData: any, i: any) => {
                                      return (
                          ...
                          {state?.Result?.OffshoreComments != null &&
                            state?.Result?.OffshoreComments != undefined &&
                            state?.Result?.OffshoreComments?.length > 0 &&
                            state?.Result?.OffshoreComments?.some(
                              (item: any) => item?.Type !== "EODReport"
                            ) && (
                              <div className="col-sm-8 pe-0 mt-2">
                                <fieldset className="border p-1">
                                  <legend className="border-bottom fs-6">
                                    Background Comments
                                  </legend>
                                  {state?.Result?.OffshoreComments != null &&
                                    state?.Result?.OffshoreComments?.length >
                                      0 &&
                                    state?.Result?.OffshoreComments?.map(
                                      (item: any, index: any) => {
                                        return (
                                          item?.Type !== "EODReport" && (
                                            <div>
                ...
                <div className="col-3 pe-0">
                  <div>
                    {state?.Result != undefined && AllListId != undefined && (
                      <CommentCard
                        siteUrl={propsValue?.siteUrl}
                        AllListId={AllListId}
                        Context={propsValue.Context}
          ...
          <section className="TableContentSection">
            ...
            <div className="row m-0">
              ...
              state?.Result?.Id != undefined &&
              state?.Result.TaskTypeTitle != "" &&
              state?.Result.TaskTypeTitle != undefined &&
              state?.Result.TaskTypeTitle != "Task" ? (
                ...
                <RadimadeTable
                  tableId="TaskProfilegit"
                  AllListId={AllListId}
                  configration={"AllAwt"}
                  SelectedSiteForTask={[listName]}
                  SelectedItem={state?.Result}
                ></RadimadeTable>
              ) : (
                ""
              )}
              ...
              {state?.Result != undefined && (
                <div className="ItemInfo mb-20" style={{ paddingTop: "15px" }}>
                  <div>
                    Created{" "}
                    <span>
                      {moment(state?.Result?.Creation).format(
                        "DD MMM YYYY HH:mm "
                      )}
                    </span>{" "}
                    by{" "}
                  ...
                  <div>
                    Last modified{" "}
                    <span>
                      {moment(state?.Result?.Modified).format(
                        "DD MMM YYYY HH:mm "
                      )}
                    </span>{" "}
                    by{" "}
                    <span className="siteColor">
                      {state?.Result?.ModifiedBy != null &&
                        state?.Result?.ModifiedBy?.Title}
                    </span>
                    ...
                    <span>
                      {itemID ? (
                        <VersionHistoryPopup
                          ...
                          context={propsValue.Context}
                          RequiredListIds={AllListId}
                          listId={state?.Result.listId}
                          siteUrls={state?.Result.siteUrl}
                          isOpen={state.isopenversionHistory}
                        />
                      ) : (
                        ""
                      )}
          ...
          {state?.imageInfo && (
            <div className="imghover" style={{ display: state.showPopup }}>
              <div className="popup">
                <div className="parentDiv">
                  <span style={{ color: "white" }}>
                    {state?.imageInfo?.ImageName}
                  </span>
                  <img
                    style={{ maxWidth: "100%" }}
                    src={state?.imageInfo?.ImageUrl}
                  ></img>
                </div>
              </div>
            </div>
          ...
          {state?.isCalloutVisible ? (
            ...
            >
              ...
              >
                <Stack className="modal-footer" gap={8} horizontal>
                  <button
          ...
          {state?.emailcomponentopen && countemailbutton == 0 && (
            <EmailComponenet
              approvalcallback={() => {
                approvalcallback();
              }}
              Context={propsValue?.Context}
              emailStatus={state?.emailComponentstatus}
              currentUser={currentUser}
              items={state?.Result}
            />
          )}
          ...
          {(isopencomonentservicepopup || isopenProjectpopup) && (
            <ServiceComponentPortfolioPopup
              ...
              Call={(DataItem: any, Type: any, functionType: any) => {
                ComponentServicePopupCallBack(DataItem, Type, functionType);
              }}
              showProject={isopenProjectpopup}
            />
          )}
          {state?.CommenttoUpdate != undefined && (
            <Panel
              onRenderHeader={onRenderCustomHeadereditcomment}
              isOpen={
                state.isEditModalOpen
                  ? state.isEditModalOpen
                  : state.isEditReplyModalOpen
              }
          ...
          {openComparePopup && (
            <ImageViewPanel
              currentUser={currentUser}
              checkedImageData={checkedImageData}
              SetOpenComparePopup={SetOpenComparePopup}
              AllImageData={state?.Result?.BasicImageInfo}
              AllListId={AllListId}
              taskUsers={taskUsers}
              taskData={state?.Result}
            />
          )}
          {openCategories && (
            <Picker
              props={state?.Result}
              selectedCategoryData={state?.Result?.TaskCategories}
              usedFor="Task-Popup"
              AllListId={props?.props}
              CallBack={CategoryCallBack}
              closePopupCallBack={() => setOpenCategories(false)}
            ></Picker>
          )}
        </div>
      </myContextValue.Provider>
