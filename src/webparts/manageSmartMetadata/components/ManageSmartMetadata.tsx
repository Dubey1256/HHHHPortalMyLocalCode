...
export default class ManageSmartMetadata extends React.Component<IManageSmartMetadataProps, IManageSmartMetadataState> {
  ...
  private async getTasks(sMetadataItem: ISmartMetadataItem) {
    ...
    let _TaskItems: any[] = [];
    const qFilterTask: string = `SharewebCategories/Title eq '${sMetadataTitle}'`;
    SITE_CONFIGURATION.forEach(siteInfo=>{
      ...
      let qExpandTask: string = "ParentTask,Events,Services,SharewebTaskType,AssignedTo,Component,ClientCategory,Author,Editor,Team_x0020_Members,Responsible_x0020_Team,SharewebCategories";
      if(siteTitle=="Master Tasks") {
        ...
        qExpandTask = "SharewebTaskType,ComponentCategory,AssignedTo,Component,Events,Services,AttachmentFiles,Author,Editor,Team_x0020_Members,SharewebComponent,SharewebCategories,Parent";
      }
      const qStrings = {
        Select: qSelectTask,
        Expand: qExpandTask,
  ...
  private getSharedWebId(taskItem: ITask) {
    ...
    const taskType: string = taskItem.SharewebTaskType ? taskItem.SharewebTaskType.Title : null;
    const taskLevelOneNum: string = taskItem.SharewebTaskLevel1No;
    const taskLevelTwoNum: string = taskItem.SharewebTaskLevel2No;
