...
import FlorarImagetabportfolio from './FlorarImageTabEventSite'
...
const ImagetabFunction = (props: any) => {
    ...
    const [chooseExistingFile, setChooseExistingFile] = useState({
        ChooseExistinglogo: [], ChooseExistingCover: [], ChooseExistingImages1: [], ChooseExistingSliderImage: []
    });
    ...
    const getAllImageData = async () => {
        ...
        if (props?.usedFor === "ImageSlider")
            var data = ["Logos", "Covers", "Page-Images","SliderImages"]
        else
        ...
        let ChooseExistingSliderImagearray: any = [];
            ...
            await web.getFolderByServerRelativeUrl(`${props?.Context?._pageContext?.web?.serverRelativeUrl}/PublishingImages/${data[i]}`).files.get()
                .then(async (dataimage: any) => {
                    try {
                        ...
                        if (data[i] == "SliderImages") {
                            ChooseExistingSliderImagearray = dataimage                           
                        }
        ...
        setChooseExistingFile({ ...chooseExistingFile, ChooseExistinglogo: ChooseExistinglogoarray, ChooseExistingCover: ChooseExistingCoverarray, ChooseExistingImages1: ChooseExistingImages1array, ChooseExistingSliderImage: ChooseExistingSliderImagearray })
    }
    ...
    const changesTabFunction = (selecttab: any) => {
        ...
        if (selecttab == "SliderImage") {
            let imagesData: any[] = [];
            let myimagedata: any[] = [];
            imagesData = chooseExistingFile?.ChooseExistingSliderImage
            if (imagesData.length > 10) {
                for (let i = 0; i < 11; i++) {
                    myimagedata?.push(imagesData[i]);
                }
    ...
    const uploadImage = async () => {
        ...
        if (selectfolder == "SliderImage") {
            selectfolder2 = "PublishingImages/SliderImages"
        }
    ...
    const searchImageUrl = (value: any) => {
        ...
        if (selectfolder == "SliderImage") {
            let searchingImageData = chooseExistingFile?.ChooseExistingSliderImage?.filter((items: any) => items?.ServerRelativeUrl == value)
            // let taskItem = {...editData};
    ...
    const loadMore = async () => {
        ...
        else if (selectfolder == "SliderImage") {
            let imagesData: any = []
            imagesData = chooseExistingFile?.ChooseExistingSliderImage;
            if (count != 0 && imagesData?.length > 0) {
                let myimagedata: any[] = [];
    ...
    return (
        <>
            <div className='d-flex pe-0'>
                {usedfor == "siteportfolio" &&
                    <div> <span onClick={() => handlecover()}>Cover</span> <span onClick={() => handleTeaser()}> Teaser</span></div>
                }
                <div className="input-group ps-1"><label className=" full-width ">Image-Url </label>
                ...
                <div className="input-group ms-3 pe-0">
                    ...
                    <Row className='pe-1'>
                        <Col sm={2} className='mt-5 pe-0'>
                            <Nav variant="pills" className="flex-column">
                                ...
                                {(props?.usedFor === "ImageSlider") && <Nav.Item>
                                    <Nav.Link eventKey="SliderImage" onClick={() => changesTabFunction("SliderImage")}> SliderImage</Nav.Link>
                                </Nav.Item>}
                                ...
                                <div className='mt-2 small'><span className="alignIcon svg__iconbox svg__icon--cross dark hreflink" onClick={() => clearImage(editData?.Item_x002d_Image?.itemCoverId)}></span>Clear Image</div>
                            </Nav>
                        </Col>
                        <Col sm={10} className='p-0'>
                            <Tab.Content>
                                    ...
                                    >
                                        <Tab eventKey="copy & paste" title="Copy & Paste">
                                            <div className='border border-top-0  p-2'>
                                                ...
                                                <div className='text-lg-end mt-2'><button className='btn btn-primary btnCol ms-1' onClick={() => uploadImage()}>Upload</button></div>
                                            </div>
                                        </Tab>
                                        <Tab eventKey="Upload" title="Upload">
                                            <div className='border border-top-0  p-2'>
                                                <div className='mt-3 ' style={{ height: "500px" }}>
                                                    ...
                                                    <div className='text-lg-end mt-2'><button className='btn btn-primary btnCol ms-1 mx-2' onClick={() => uploadImage()}>Upload</button></div>
                                                </div>
                                            ...
                                            <div className='border border-top-0 ImageSec p-2 scrollbar maXh-500 hreflink'>
                                                <div className='clearfix'>
                                                    <div className="input-group ">
                                                        {/* <label className=" full-width "><ShowImagesOOTB Context={props.Context}></ShowImagesOOTB></label> */}
                                ...
                                <Tab.Pane eventKey="Images1" className='p-0'>
                                    <Tabs defaultActiveKey="copy & paste" transition={false} id="noanim-tab-example" className="">
                                        <Tab eventKey="copy & paste" title="Copy & Paste">
                                            <div className='border border-top-0  p-2'>
                                                <div className="input-group "><label className=" full-width ">Image Name</label>
                                                    <input type="text" className="form-control" value={props?.EditdocumentsData?.Title} placeholder='image Name' />
                                        ...
                                        <Tab eventKey="Choose from existing (0)" title={`Choose from existing (${chooseExistingFile?.ChooseExistingImages1?.length})`} >
                                            <div className='border border-top-0 ImageSec p-2 scrollbar maXh-500 hreflink'>
                                                ...
                                                <div>
                                                    {loadedImages.map((img) => (img !== undefined && <img src={img?.ServerRelativeUrl} onClick={() => ExistingImageUpload(img)} />))}
                                                </div>
                                                <button onClick={() => loadMore()} type='button'>Load More</button>
                                            </div>
                                        </Tab>
                                ...
                                <Tab.Pane eventKey="SliderImage" className='p-0'>
                                    <Tabs defaultActiveKey="copy & paste" transition={false} id="noanim-tab-example" className="">
                                        <Tab eventKey="copy & paste" title="Copy & Paste">
                                            <div className='border border-top-0  p-2'>
                                                <div className="input-group "><label className=" full-width ">Image Name</label>
                                        ...
                                        <Tab eventKey="Choose from existing (0)" title={`Choose from existing (${chooseExistingFile?.ChooseExistingSliderImage?.length})`} >
                                            <div className='border border-top-0 ImageSec p-2 scrollbar maXh-500 hreflink'>
                                                <div className='clearfix'>
                                                    <div className="input-group ">
                                                        {/* <label className=" full-width "><ShowImagesOOTB Context={props.Context}></ShowImagesOOTB></label> */}
