...
const EditContactPopup = (props: any) => {
    ...
    const openSmartTaxonomyCategories = (item: any) => {
        setStatus({
            ...status, orgPopup: false,
            taxanomypopup: false,
            CategoriesPopup: true
        })
    }
    const getContactDetails = async () => {
        try {
            //props?.allListId?.TeamContactSearchlistIds
            const select = "WorkCity,WorkFax ,WorkAddress,Id,WorkCountry,Email,FullName,WorkFax,ItemCover,SmartActivities/Id,SmartActivities/Title,SmartCategories/Id,SmartCategories/Title,Divisions,Attachments,Categories,Company,OVMitgliedBeginn,AnsprechpartnerimOV,VerbindungzuOV,Mitgliederverband,Listeseit,ListeEnde,JobTitle,FirstName,Title,Suffix,WebPage,IM,ol_Department,WorkPhone,CellPhone,HomePhone,WorkZip,Office,Comments,Created,Modified,Author/Name,Author/Title,Editor/Name,Editor/Title";
            const query = `Id eq ${itemId}`;
            const data = await webs.lists.getById("dcd6f07e-c6c1-42fd-a191-d5664fa5c930").items.select(select).expand('Author', 'Editor', 'SmartActivities', 'SmartCategories').filter(query).get();
    ...
    const CloseSmarttaxonomypopup = useCallback((data: any) => {
        setStatus({ ...status, taxanomypopup: false, CategoriesPopup: false })
        if (data != undefined) {
    ...
    const UpdateContact = function (Item: any) {
        try {
            ...
            let postData = {
                ...
                OVMitgliedBeginn: contactDetails?.OVMitgliedBeginn,
                AnsprechpartnerimOV: contactDetails?.AnsprechpartnerimOV,
                VerbindungzuOV: contactDetails?.VerbindungzuOV,
                Mitgliederverband: contactDetails?.Mitgliederverband,
                Listeseit: contactDetails?.Listeseit,
                ListeEnde: contactDetails?.ListeEnde,
            ...
            let updatedData = webs.lists.getById("dcd6f07e-c6c1-42fd-a191-d5664fa5c930").items.getById(Item.Id).update(postData)
    ...
    const removeItem = async () => {
        ...
        try {
            if (confirm("Are you sure, you want to delete this?")) {
                await webs.lists.getById("dcd6f07e-c6c1-42fd-a191-d5664fa5c930").items.getById(itemId).recycle().then((e) => {
    ...
    return (
        <>
                                                            ...
                                                            <div className="col-sm-6 pe-0">
                                                                <div className="col-sm-12">
                                                                    <div className='input-group'>
                                                                        ...
                                                                        <input  type="text"  className="form-control"  placeholder="Search Category Here"  />
            ...
            {status.CategoriesPopup ? <Smarttaxonomypopup popupName="Contact Categories" selectedCountry={currentCountry} callBack={CloseSmarttaxonomypopup} data={AllCategoryData} updateData={contactDetails} /> : null}
        </>
