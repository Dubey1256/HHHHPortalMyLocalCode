...
const ContactMainPage = (props: any) => {
    ...
    const callBackData = (data: any) => {
        if (data != undefined) {
            setIsDisabled(false);
            data.isSelect = true;
            bulkItemEmail.push(data);
            setUserEmails(bulkItemEmail);
        } else {
            bulkItemEmail = [];
            setUserEmails([]);
            setIsDisabled(true);
    ...
    const customTableHeaderButtons: any = (
        <>
            ...
            {allSite?.MainSite && <button className={isDisabled ? 'btnCol btn btn-primary' : "btnCol btn btn-primary"} onClick={() => setAddToLocalDBStatus(true)} disabled={isDisabled}>Add Contact To The Local Database</button>}
    ...
    const customTableHeaderInstituteButtons: any = (
        <>
            {allSite?.MainSite && <button className={isDisabled ? 'btnCol btn btn-primary' : "btnCol btn btn-primary"} onClick={() => setAddToLocalDBStatus(true)} disabled={isDisabled}>Tag Institution</button>}
            <button className='btnCol btn btn-primary' onClick={() => setCreateInstituteStatus(true)}>Create Institution</button>
        </>
    )
    // **************End *************************************
    return (
        <myContextValue.Provider value={{ ...myContextValue, allSite: allSite, allListId: allListId, divisionData: divisionData, setDivisionData, loggedInUserName: props.props?.userDisplayName, InstitutionAllData: institutionData }}>
            <div className='contact-section'>
                        ...
                        <div className="border border-top-0 clearfix p-3 tab-content " id="myTabContent">
                            <div className="tab-pane show active" id="Contacts" role="tabpanel" aria-labelledby="Contacts">
                                <div className='Alltable'>
                                    <GlobalCommanTable columns={allSite?.HrSite ? hrColumns : columns} data={searchedData} showHeader={true} customHeaderButtonAvailable={true} customTableHeaderButtons={customTableHeaderButtons}
                            ...
                            <div className="tab-pane" id="Institution" role="tabpanel" aria-labelledby="Institution">
                                <div className='Alltable'>
                                    <GlobalCommanTable columns={Inscolumns} data={searchedInstituteData} showHeader={true} customHeaderButtonAvailable={true} customTableHeaderButtons={customTableHeaderInstituteButtons}
                                        callBackData={callBackData}
                                    />
                                </div>
                            </div>
                ...
                {EditContactStatus ? <HHHHEditComponent props={EditContactData} siteurl={props?.props?.Context?._pageContext?.web?.absoluteUrl} callBack={ClosePopup} /> : null}
                ...
                {EditInstitutionStatus ? <EditInstitutionPopup siteurl={props?.props?.Context?._pageContext?.web?.absoluteUrl} props={editInstitutionData} callBack={ClosePopup} /> : null}
            </div>
