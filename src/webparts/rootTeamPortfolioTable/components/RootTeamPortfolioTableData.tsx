...
import RootLevelSmartFilter from "../../../globalComponents/RootSmartFilter/RootLevelSmartFilter";
let isUpdated: any = "";
let ProjectData: any = [];
...
let AllRootMAsterTaskData: any = [];
let AllRootTaskData: any = [];
...
const RootTeamPortfolioTableData = (props: any) => {
            ...
            try {
                ...
                smartmeta?.map(async (item: any) => {
                    if (item?.Title == 'RootDashboardConfig') {
                        ...
                        portfolioTypeData = AllSubSitePortfolioTypeData?.current
                    }
                })
    ...
    const GetSmartmetadata = async (config: any) => {
        ...
        try {
            ...
            await Promise.all(smartmetaDetails.map(async (newtest: any) => {
                newtest.siteName = config.siteName;
    ...
    const smartFiltercallBackData = React.useCallback((filterDataBackup, updatedSmartFilter, smartTimeTotal, flatView) => {
        let filterData: any = []
        try {
            filterData = JSON.parse(JSON.stringify(filterDataBackup));
        } catch (error) {
            console.log("backup Json parse error smartFiltercallBackData function");
        }
        smartAllFilterData = AllSiteSubSiteListDeatails.current.reduce((acc: any, x: any) => {
            acc[x.siteName] = filterData.filter((e: any) => e.siteName === x.siteName);
            return acc;
        }, {});
        console.log("updatedSmartFilter", updatedSmartFilter, "filterData", filterData);
        if (filterData?.length > 0 && updatedSmartFilter === false) {
            isColumnDefultSortingAsc = false;
            DataPrepareForCSFAWT();
        } else if (filterData?.length > 0 && updatedSmartFilter === true) {
    ...
    const GetComponents = async (config: any) => {
        ...
        try {
            AllRootMAsterTaskData = AllRootMAsterTaskData.concat(JSON.parse(JSON.stringify(componentDetails)))
        } catch (error) {
        ...
        if (siteConfigForAllSubSite.current[config3?.siteName] != undefined && siteConfigForAllSubSite.current[config3?.siteName]?.length > 0) {
            await Promise.all(map(siteConfigForAllSubSite.current[config3?.siteName], async (config: any) => {
                ...
                if (Counter == siteConfigForAllSubSite?.current[config3?.siteName]?.length) {
                    ...
                    try {
                        AllRootTaskData = AllRootTaskData.concat(JSON.parse(JSON.stringify(AllTasksData)))
                    } catch (error) {
                        console.log("backup Json parse error Page Loade Task Data");
                    }
    ...
    const componentGrouping = async (portId: any, index: any, config: any) => {
        ...
        let AllProtFolioData = smartAllFilterData?.[config?.siteName]?.filter((comp: any) => comp?.PortfolioType?.Id === portId && comp.TaskType === undefined);
    ...
    const componentActivity = (levelType: any, items: any, config: any) => {
        ...
        if (items?.Id != undefined) {
            findActivity = smartAllFilterData[config?.siteName]?.filter((elem: any) => elem?.TaskType?.Id === levelType.Id && elem?.Portfolio?.Id === items?.Id);
            findTasks = smartAllFilterData[config?.siteName]?.filter((elem1: any) => elem1?.TaskType?.Id != levelType.Id && (elem1?.ParentTask?.Id === 0 || elem1?.ParentTask?.Id === undefined) && elem1?.Portfolio?.Id === items?.Id);
        }
        ...
        findActivity?.forEach((act: any) => {
            act.subRows = [];
            let worstreamAndTask = smartAllFilterData[config?.siteName]?.filter((taskData: any) => taskData?.ParentTask?.Id === act?.Id && taskData?.siteType === act?.siteType)
            ...
            worstreamAndTask?.forEach((wrkst: any) => {
                ...
                let allTasksData = smartAllFilterData[config?.siteName]?.filter((elem: any) => elem?.ParentTask?.Id === wrkst?.Id && elem?.siteType === wrkst?.siteType);
        ...
        <div id="ExandTableIds" style={{}}>
            ...
            <section className="Tabl1eContentSection row taskprofilepagegreen">
                ...
                <div className="togglecontent mt-1">
                    {filterCounters == true ? <RootLevelSmartFilter AllRootUsersData={AllSubSiteTaskUser?.current} allMetaRootMeteData={AllSubSiteAllMetadata?.current} openTableSettingPopup={childRef?.current?.openTableSettingPopup} setSmartFabBasedColumnsSetting={childRef?.current?.setSmartFabBasedColumnsSetting} LoadAllSiteTasksAllData={[]} AllSiteTasksDataLoadAll={[]} IsUpdated={IsUpdated} IsSmartfavorite={[]} IsSmartfavoriteId={""} ProjectData={ProjectData} portfolioTypeData={portfolioTypeData} setLoaded={setLoaded} AllSiteTasksData={AllRootTaskData} AllMasterTasksData={AllRootMAsterTaskData} SelectedProp={{}} ContextValue={{}} smartFiltercallBackData={smartFiltercallBackData} portfolioColor={""} /> : ''}
                </div>
