...
export default class parseRecurrentEvent {
  ...
  public parseEvents(events: any[], start: any, end: any, offset: number) {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (events[i].RecurrenceData.indexOf('<windowEnd>') != -1) {
        ...
        this.endDate = new Date(moment.utc(wDtEnd).utcOffset(offset).format("LLL").toString());
      }
  ...
  public RecurrenceExceptionExists(masterSeriesItemId: string, date: string | Date) {
          ...
          for (let i = initDay; i < 7; i++) {
            if (arr.indexOf(wd[i]) != -1 && (rTotal > total || rTotal == 0)) {
              ...
              if ((new Date(init)).getTime() >= start.getTime()) {
                ...
                if (this.endDate==null || moment(ed).format("L")<=moment(this.endDate).format("L")) {
                  ni.EndDate = ed;
                  ni.fRecurrence = false;
                  ni.Id = e.Id;
                  ni.ID = e.Id;
