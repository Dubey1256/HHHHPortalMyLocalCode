    ...
    <>
      <div>
        <div className="alignCenter mb-2 justify-content-between">
          {MyNotes?.config?.WebpartTitle != undefined && <span className='fw-bold'>{MyNotes?.config?.WebpartTitle}({myNoteData.length > 0 ? myNoteData.length : 0})</span>}
          <span className="alignCenter">
            {MyNotes?.IsShowConfigBtn && <span className="svg__iconbox svg__icon--setting hreflink" title="Manage Configuration" onClick={(e) => OpenConfigPopup(MyNotes?.config)}></span>}
            {myNoteData.length == 0 &&
