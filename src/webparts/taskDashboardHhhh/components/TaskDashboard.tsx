...
const TaskDashboard = (props: any) => {
  ...
  React.useEffect(() => {
    ...
    if (
      props?.props?.TaskTimeSheetListID != undefined &&
      props?.props?.TaskTimeSheetListID != ""
    ) {
      setIsTimeEntry(true);
    } else {
  ...
  const createDisplayDate = () => {
    ...
    displayDate.day = displayDate.fullDate.toLocaleString("en-GB", {
      weekday: "long"
    });
    displayDate.date = displayDate.fullDate.toLocaleString("en-GB", {
      day: "numeric"
    });
    displayDate.month = displayDate.fullDate.toLocaleString("en-GB", {
      month: "long"
    });
    today = displayDate;
  };
  ...
  const checkTimeEntrySite = (timeEntry: any) => {
    ...
    result = AllTasks?.filter((task: any) => {
      if (
        timeEntry[`Task${task?.siteType}`] != undefined &&
        task?.Id == timeEntry[`Task${task?.siteType}`]?.Id
      ) {
        return task;
      }
    });
  ...
  const currentUserTimeEntryCalculation = () => {
    ...
    const allTimeCategoryTime = timesheetDistribution.reduce(
      (totals, start) => {
        const startDate = getStartingDate(start);
        const startDateMidnight = new Date(startDate.setHours(0, 0, 0, 0));
        ...
        const total = AllTaskTimeEntries?.reduce((acc: any, timeEntry: any) => {
          if (timeEntry?.AdditionalTimeEntry) {
            ...
            const taskTime = AdditionalTime.reduce(
              (taskAcc: any, filledTime: any) => {
                const [day, month, year] = filledTime?.TaskDate?.split("/");
                const timeFillDate = new Date(+year, +month - 1, +day);
              ...
              .then((data: any) => {
                ...
                smartmeta?.map((task: any) => {
                  try {
                    ...
                    if (
                      task?.EstimatedTimeDescription != undefined &&
                      task?.EstimatedTimeDescription != "" &&
                      task?.EstimatedTimeDescription != null
                    ) {
                      EstimatedDesc = JSON.parse(
                        task?.EstimatedTimeDescription
                      );
                    }
                    task.HierarchyData = [];
                    ...
                    const isBottleneckTask = checkUserExistence(
                      "Bottleneck",
                      task?.TaskCategories
                    );
                    const isImmediate = checkUserExistence(
                      "Immediate",
                      task?.TaskCategories
                    );
                    const isEmailNotification = checkUserExistence(
                      "Email Notification",
                      task?.TaskCategories
                    );
                    const isCurrentUserApprover =
                      task?.ApproverIds?.includes(currentUserId);
                    ...
                    if (
                      parseInt(task.PriorityRank) >= 8 &&
                      parseInt(task.PriorityRank) <= 10
                    ) {
            ...
            if (arraycount === currentCount) {
              ...
              if (query != undefined && query != null && query != "") {
                ...
                if (userFound == false) {
                  if (
                    confirm(
                      "User Not Found , Do you want to continue to your Dashboard?"
                    )
                  ) {
  ...
  const filterCurrentUserTask = () => {
    ...
    if (
      ...
      AllTasks?.map((task: any) => {
        ...
        const isEmailNotfication = checkUserExistence(
          "Email Notification",
          task?.TaskCategories
        );
        const isBottleneckTask = checkUserExistence(
  ...
  const columns = React.useMemo(
    () => [
        ...
        Cell: ({ row }: any) => (
          <span>
            <ReactPopperTooltipSingleLevel
              ...
              row={row?.original}
              singleLevel={true}
              masterTaskData={MyAllData}
              AllSitesTaskData={AllSitesTask}
      ...
      },
      {
        ...
        Cell: ({ row }: any) => (
          <span>
            {row?.original?.EstimatedTime != undefined
  ...
  const UpdateTaskStatus = async (task: any) => {
    ...
    let AssignedUsers =
      task?.AssignedToIds?.length > 0 ? task?.AssignedToIds : [];
    let postworkingThisWeekTask =
      task?.workingThisWeek != undefined ? task.workingThisWeek : false;
  ...
  const GetMetaData = async () => {
    if (AllListId?.SmartMetadataListID != undefined) {
      ...
      let select: any = "";
      if (
        AllListId?.TaskTimeSheetListID != undefined &&
        AllListId?.TaskTimeSheetListID != ""
      ) {
        select =
          "Id,IsVisible,ParentID,Title,SmartSuggestions,Description,Configurations,TaxType,Description1,Item_x005F_x0020_Cover,Color_x0020_Tag,listId,siteName,siteUrl,SortOrder,SmartFilters,Selectable,Parent/Id,Parent/Title";
      } else {
        select =
          "Id,IsVisible,ParentID,Title,SmartSuggestions,Configurations,TaxType,Item_x005F_x0020_Cover,Color_x0020_Tag,listId,siteName,siteUrl,SortOrder,SmartFilters,Selectable,Parent/Id,Parent/Title";
      }
      let TaxonomyItems = [];
      try {
        ...
        siteConfig = smartmeta.filter((data: any) => {
          if (
            data?.IsVisible &&
            data?.TaxType == "Sites" &&
            data?.Title != "Master Tasks" &&
            data?.listId != undefined &&
            data?.listId?.length > 32
          ) {
            return data;
          }
        });
    ...
    if (AllListId?.TaskUsertListID != undefined) {
      try {
        ...
        taskUser = await web.lists
          ...
          .items.select(
            "Id,UserGroupId,Suffix,IsActive,Title,Email,SortOrder,Role,showAllTimeEntry,Company,Group,ParentID1,Status,Item_x0020_Cover,AssingedToUserId,isDeleted,AssingedToUser/Title,AssingedToUser/Id,AssingedToUser/EMail,ItemType,Approver/Id,Approver/Title,Approver/Name&$expand=AssingedToUser,Approver"
          )
  ...
  const handleDrop = (destination: any) => {
    if (
      currentUserId == currentUserData?.AssingedToUserId ||
      currentUserData?.showAllTimeEntry == true
    ) {
      ...
      if (
        ...
        (task?.workingThisWeek == false || task?.workingThisWeek == undefined)
      ) {
        ...
        todayTasks = todayTasks.filter(
          (taskItem) => taskItem.TaskID != dragedTask.taskId
        );
      ...
      if (
        destination == "workingToday" &&
        (task?.IsTodaysTask == false || task?.IsTodaysTask == undefined)
      ) {
        task.IsTodaysTask = true;
        task.workingThisWeek = false;
        UpdateTaskStatus(task);
        todayTasks.push(task);
        thisWeekTask = thisWeekTask.filter(
          (taskItem) => taskItem.TaskID != dragedTask.taskId
        );
        allTasks = allTasks.filter(
          (taskItem) => taskItem.TaskID != dragedTask.taskId
      ...
      if (
        ...
        (task?.IsTodaysTask == true || task?.workingThisWeek == true)
      ) {
        task.IsTodaysTask = false;
        task.workingThisWeek = false;
        UpdateTaskStatus(task);
      ...
      if (destination == "UnAssign") {
        task.IsTodaysTask = false;
        task.workingThisWeek = false;
        task.AssignedToIds = task?.AssignedToIds?.filter(
  ...
  const shareTaskInEmail = (input: any) => {
    ...
    let confirmation = confirm(
      "Your" +
        ...
        input +
        " " +
        "will be automatically shared with your approver" +
        " " +
        "(" +
        userApprover +
        ")" +
    ...
    if (confirmation) {
      if (input == "today working tasks") {
        ...
        tasksCopy?.map((item: any) => {
          ...
          text =
            "<tr>" +
            '<td style="line-height:24px;font-size:13px;padding:15px;">' +
            item.siteType +
            "</td>" +
            '<td style="line-height:24px;font-size:13px;padding:15px;">' +
            item.TaskID +
            "</td>" +
            '<td style="line-height:24px;font-size:13px;padding:15px;">' +
            '<p style="margin-top:0px; margin-bottom:2px;font-size:14px; color:#333;">' +
            "<a href =" +
            item.siteUrl +
        ...
        body =
          ...
          "<thead>" +
          "<tr>" +
          '<th style="line-height:24px;font-size:15px;padding:10px;" bgcolor="#f5f5f5">' +
          "Site" +
          "</th>" +
          '<th style="line-height:24px;font-size:15px;padding:10px;" bgcolor="#f5f5f5">' +
          "ID" +
          ...
          "</table>" +
          "<p>" +
          "For the complete Task Dashboard of " +
          currentLoginUser +
          " click the following link:" +
          "<a href =" +
          `${AllListId?.siteUrl}/SitePages/TaskDashboard.aspx?UserId=` +
          currentUserId +
          '><span style="font-size:13px; font-weight:600">' +
          `${AllListId?.siteUrl}/SitePages/TaskDashboard.aspx?UserId=` +
          currentUserId +
    ...
    if (input == "today time entries") {
      ...
      weeklyTimeReport.map((item: any) => {
        ...
        text =
          "<tr>" +
          '<td style="line-height:18px;font-size:13px;padding:15px;;">' +
          item?.siteType +
          "</td>" +
          '<td style="line-height:18px;font-size:13px;padding:15px;;">' +
          item?.TaskID +
      ...
      body =
        ...
        "<p>" +
        "<a href =" +
        `${AllListId?.siteUrl}/SitePages/UserTimeEntry.aspx?userId=${currentUserId}` +
        ">Click here to open the Complete time entry" +
        "</a>" +
        "</p>" +
        "<p>" +
        "<a href =" +
        ...
        currentUserId +
        ">" +
        "Click here to open Task Dashboard of " +
        currentLoginUser +
        "</a>" +
        "</p>";
      body = body.replaceAll(">,<", "><").replaceAll(",", "");
  ...
  const sendAllWorkingTodayTasks = () => {
    let text = "";
    let to: any = [
      "ranu.trivedi@hochhuth-consulting.de",
      "prashant.kumar@hochhuth-consulting.de",
      "abhishek.tiwari@hochhuth-consulting.de",
      "deepak@hochhuth-consulting.de"
    ];
    let finalBody: any = [];
    let userApprover = "";
    let taskUsersGroup = groupedUsers;
    let confirmation = confirm(
      "Are you sure you want to share the working today task of all team members?"
    );
    if (confirmation) {
      ...
      taskUsersGroup?.map((userGroup: any) => {
        ...
        if (
          userGroup.Title == "Junior Developer Team" ||
          userGroup.Title == "Senior Developer Team" ||
          userGroup.Title == "Design Team" ||
          userGroup.Title == "QA Team" ||
          userGroup.Title == "Smalsus Lead Team" ||
          userGroup.Title == "Business Analyst"
        ) {
          ...
          userGroup?.childs?.map((teamMember: any) => {
            if (
              !onLeaveEmployees.some(
                (emp: any) => emp == teamMember?.AssingedToUserId
              )
            ) {
              let body: any = "";
              let body1: any = [];
              let tasksCopy: any = [];
              tasksCopy = filterCurrentUserWorkingTodayTask(
                teamMember?.AssingedToUserId
              );
              if (tasksCopy?.length > 0) {
                tasksCopy?.map((item: any) => {
                  ...
                  if (item.DueDate != undefined) {
                    item.TaskDueDatenew = Moment(item.DueDate).format(
                      "DD/MM/YYYY"
                    );
                  }
                  if (
                    item.TaskDueDatenew == undefined ||
                    item.TaskDueDatenew == ""
                  )
                    item.TaskDueDatenew = "";
                  if (item.Categories == undefined || item.Categories == "")
                ...
                body =
                  "<h3><strong>" +
                  teamMember?.Title +
                  ` (${teamMember?.Group != null ? teamMember?.Group : ""})` +
                  "</strong></h3>" +
                  '<table style="border: 1px solid #ccc;" border="1" cellspacing="0" cellpadding="0" width="100%">' +
                  "<thead>" +
                  "<tr>" +
          ...
          let TeamTitle =
            "<h2><strong>" + userGroup.Title + "</strong></h2>" + teamsTaskBody;
      ...
      let sendAllTasks =
        ...
        "<p>" +
        "<a href =" +
        `${AllListId?.siteUrl}/SitePages/Project-Management-Overview.aspx` +
        ">Click here for flat overview of the today's tasks: " +
        "</a>" +
        "</p>" +
        "</span>" +
        finalBody +
  ...
  return (
    <>
      ...
      <div
        className="TaskDashboardPage Dashboardsecrtion"
      ...
      >
        <div className={updateContent ? "dashboard-colm" : "dashboard-colm"}>
          <aside className="sidebar">
            ...
            <section className="sidebar__section sidebar__section--menu">
              <nav className="nav__item">
                <ul className="nav__list mb-0">
                  ...
                  <li className="nav__item  pb-1 pt-0"></li>
                  {currentUserData?.Title == "Deepak Trivedi" ||
                  currentUserData?.Title == "Ranu Trivedi" ||
                  currentUserData?.Title == "Abhishek Tiwari" ||
                  currentUserData?.Title == "Prashant Kumar" ? (
                    <a
                      className="text-white hreflink"
                      onClick={() => sendAllWorkingTodayTasks()}
                    >
            ...
            <section className="sidebar__section sidebar__section--menu">
              ...
              currentUserData?.showAllTimeEntry == true ? (
                <>
                  <div
                    onDrop={(e: any) => handleDrop("UnAssign")}
                    className="mb-2 nontag text-center drophere nav__text"
              ...
              <nav className="nav__item">
                <ul className="nav__list text-center">
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "AllImmediateTasks"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("AllImmediateTasks");
                    }}
                  >
                    Immediate Tasks
                  </li>
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "AllEmailTasks"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("AllEmailTasks");
                    }}
                  >
                    Email-Notification
                  </li>
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "AllPriorityTasks"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("AllPriorityTasks");
                    }}
                  >
                    Priority Tasks
                  </li>
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "allApproverView"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("allApproverView");
                    }}
                  >
                    Approver Tasks
                  </li>
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "allBottlenecks"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("allBottlenecks");
                    }}
                  >
                    Bottleneck Tasks
                  </li>
                  ...
                  <li
                    id="DefaultViewSelectId"
                    className={
                      currentView == "sharewebTasks"
                        ? "nav__text bg-secondary mb-1 hreflink"
                        : "nav__text mb-1 bg-shade hreflink "
                    }
                    onClick={() => {
                      setCurrentView("sharewebTasks");
                    }}
                  >
                    Shareweb Tasks
                  </li>
            ...
            <section
              className="sidebar__section sidebar__section--menu"
                  ...
                  {groupedUsers?.map((filterItem: any, index: any) => {
                    if (filterItem?.childs?.length > 0) {
                      return (
                        <li
                          id="DefaultViewSelectId"
                          onClick={() => toggleTeamUsers(index)}
                          className={
                            updateContent
                              ? "nav__text hreflink bg-shade  mb-1 "
                              : "nav__text bg-shade hreflink mb-1 "
                          }
                        >
                          {filterItem?.Title}
                          {filterItem?.expanded ? (
                            <span className="svg__iconbox svg__icon--arrowDown  float-start me-1 "></span>
                          ) : (
                            <span className="svg__iconbox svg__icon--arrowRight  float-start me-1"></span>
                          )}
                                ...
                                return (
                                  ...
                                  >
                                    <a
                                      ...
                                      target="_blank"
                                      data-interception="off"
                                      title={childUsers.Title}
                                      onClick={() =>
                                        changeSelectedUser(childUsers)
                                      }
                                    >
                                      {childUsers.Title}
                                    </a>
          ...
          <div
            className={
              updateContent
                ? "dashboard-content ps-2 full-width"
                : "dashboard-content ps-2 full-width"
            }
          >
            {currentView == "Home" ? (
              <article className="row">
                ...
                <div className="col-md-12">
                  <details
                    open
                    onDrop={(e: any) => handleDrop("workingToday")}
                    onDragOver={(e: any) => e.preventDefault()}
                  >
                    <summary>
                      {" "}
                      ...
                      {currentUserId == currentUserData?.AssingedToUserId ? (
                        <span
                          className="align-autoplay d-flex float-end"
                          onClick={() =>
                            shareTaskInEmail("today working tasks")
                          }
                        >
                          <span className="svg__iconbox svg__icon--mail mx-1"></span>
                          Share Today Working Tasks
                        </span>
                      ) : (
                        ""
                      )}
                    ...
                    <div className="AccordionContent mx-height">
                      {workingTodayTasks?.length > 0 ? (
                        ...
                        >
                          ...
                          {pageToday?.length > 0 ? (
                            <tbody
                              className={updateContent ? "p-0" : ""}
                              {...getTableBodyPropsToday}
                            >
                  ...
                  <details
                    onDrop={(e: any) => handleDrop("thisWeek")}
                    onDragOver={(e: any) => e.preventDefault()}
                  >
                    <summary>
                      {" "}
                      Working This Week Tasks {"(" +
                        pageWeek?.length +
                        ")"}{" "}
                    </summary>
                    <div className="AccordionContent mx-height">
                  ...
                  <details>
                    <summary>
                      {" "}
                      Immediate Tasks {"(" + pageImmediate?.length + ")"}{" "}
                    </summary>
                    <div className="AccordionContent mx-height">
                  ...
                  <details
                    onDrop={(e: any) => handleDrop("AllTasks")}
                    onDragOver={(e: any) => e.preventDefault()}
                  >
                    <summary>
                      Assigned Tasks{" "}
                      {"(" + backupTaskArray?.AllAssignedTasks?.length + ")"}
                    </summary>
                    <div className="AccordionContent mx-height">
                  ...
                  isTimeEntry == true ? (
                    <>
                        ...
                        <dl>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="Yesterday"
                              name="date"
                              checked={selectedTimeReport == "Yesterday"}
                              onClick={() => currentUserTimeEntry("Yesterday")}
                            />{" "}
                            Yesterday
                          </dt>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="Today"
                              name="date"
                              checked={selectedTimeReport == "Today"}
                              onClick={() => currentUserTimeEntry("Today")}
                            />{" "}
                            Today
                          </dt>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="This Week"
                              name="date"
                              checked={selectedTimeReport == "This Week"}
                              onClick={() => currentUserTimeEntry("This Week")}
                            />{" "}
                            This Week
                          </dt>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="Last Week"
                              name="date"
                              checked={selectedTimeReport == "Last Week"}
                              onClick={() => currentUserTimeEntry("Last Week")}
                            />{" "}
                            Last Week
                          </dt>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="This Month"
                              name="date"
                              checked={selectedTimeReport == "This Month"}
                              onClick={() => currentUserTimeEntry("This Month")}
                            />{" "}
                            This Month
                          </dt>
                          <dt className="SpfxCheckRadio">
                            <input
                              ...
                              value="Last Month"
                              name="date"
                              checked={selectedTimeReport == "Last Month"}
                              onClick={() => currentUserTimeEntry("Last Month")}
                            />{" "}
                            Last Month
                          </dt>
                        </dl>
                      ...
                      <div>
                        <a
                          className="accordion-Btn-right mt-1"
                          title="Refresh Time Entries"
                          onClick={() => {
                            loadAllTimeEntry();
                          }}
                        >
                          <span className="svg__iconbox svg__icon--refresh mx-1"></span>
                        </a>
                        <details open>
                          {timeEntryTotal > 1 ? (
                            <summary>
                              {selectedTimeReport}'s Time Entry{" "}
                              {"(" + timeEntryTotal.toFixed(2) + " Hours)"}
                              {currentUserId ==
                                currentUserData?.AssingedToUserId &&
                              selectedTimeReport == "Today" ? (
                                <span
                                  className="align-autoplay d-flex float-end me-5"
                                  onClick={() =>
                                    shareTaskInEmail("today time entries")
                                  }
                                >
                                  <span className="svg__iconbox svg__icon--mail mx-1"></span>
                                  Share {selectedTimeReport}'s Time Entry
                                </span>
                              ) : (
                                ""
                              )}
                            ...
                            <summary>
                              ...
                              selectedTimeReport == "Today" ? (
                                ...
                                >
                                  <span className="svg__iconbox svg__icon--mail mx-1 me"></span>
                                  Share {selectedTimeReport}'s Time Entry
                                </span>
                              ) : (
                                ""
                          ...
                          <div className="AccordionContent mx-height timeEntryReport">
                            {weeklyTimeReport?.length > 0 ? (
                              <Table
                                ...
                                hover
                                {...getTablePropsApprover()}
                              >
                                <thead className="fixed-Header">
                                  {headerGroupsTimeReport?.map(
                                    (headerGroup: any) => (
                                      <tr
                    ...
                    {AllBottleNeck?.length > 0 ? (
                      <>
                        ...
                        >
                          ...
                          {pageAllBottle?.length > 0 ? (
                            <tbody {...getTableBodyPropsAllBottle()}>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageAllBottle()}
                                disabled={!canPreviousPageAllBottle}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageAllBottle()}
                                disabled={!canNextPageAllBottle}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                    ...
                    {AllSitesTask?.length > 0 ? (
                      <>
                        ...
                        >
                          ...
                          {pageAllSite?.length > 0 ? (
                            <tbody {...getTableBodyPropsAllSite()}>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageAllSite()}
                                disabled={!canPreviousPageAllSite}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageAllSite()}
                                disabled={!canNextPageAllSite}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                      ...
                      <>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageApprover()}
                                disabled={!canPreviousPageApprover}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageApprover()}
                                disabled={!canNextPageApprover}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                    ...
                    {AllPriorityTasks?.length > 0 ? (
                      <>
                        ...
                        <Table
                          ...
                          hover
                          {...getTablePropsAllPriority()}
                        >
                          <thead className="fixed-Header">
                            {headerGroupsAllPriority?.map(
                              (headerGroup: any) => (
                                <tr {...headerGroup.getHeaderGroupProps()}>
                                  {headerGroup.headers.map((column: any) => (
                                    <th
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageAllPriority()}
                                disabled={!canPreviousPageAllPriority}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageAllPriority()}
                                disabled={!canNextPageAllPriority}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                    ...
                    {sharewebTasks?.length > 0 ? (
                      <>
                        ...
                        >
                          ...
                          {pageSharewebTask?.length > 0 ? (
                            <tbody {...getTableBodyPropsSharewebTask()}>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageSharewebTask()}
                                disabled={!canPreviousPageSharewebTask}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageSharewebTask()}
                                disabled={!canNextPageSharewebTask}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                    ...
                    {AllImmediateTasks?.length > 0 ? (
                      <>
                        <Table
                          ...
                          hover
                          {...getTablePropsAllImmediate()}
                        >
                          <thead className="fixed-Header">
                            {headerGroupsAllImmediate?.map(
                              (headerGroup: any) => (
                                <tr {...headerGroup.getHeaderGroupProps()}>
                                  {headerGroup.headers.map((column: any) => (
                                    <th
                          ...
                          {pageAllImmediate?.length > 0 ? (
                            <tbody {...getTableBodyPropsAllImmediate()}>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageAllImmediate()}
                                disabled={!canPreviousPageAllImmediate}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageAllImmediate()}
                                disabled={!canNextPageAllImmediate}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
                    ...
                    {AllEmailTasks?.length > 0 ? (
                      <>
                        ...
                        >
                          ...
                          {pageAllEmail?.length > 0 ? (
                            <tbody {...getTableBodyPropsAllEmail()}>
                        ...
                        <nav className="pull-right">
                          <Pagination>
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => previousPageAllEmail()}
                                disabled={!canPreviousPageAllEmail}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleLeft aria-hidden={true} />
                                </span>
                            ...
                            <PaginationItem>
                              <PaginationLink
                                onClick={() => nextPageAllEmail()}
                                disabled={!canNextPageAllEmail}
                              >
                                <span aria-hidden={true}>
                                  <FaAngleRight aria-hidden={true} />
                                </span>
          ...
          <div>
            {isOpenEditPopup ? (
              <EditTaskPopup
                ...
                Items={passdata}
                pageName="TaskDashBoard"
                Call={editTaskCallBack}
              />
            ) : (
              ""
            )}
