   ...
   // const [tablecontiner, settablecontiner]: any = React.useState("hundred");
    ...
    return (
        <>
            {showHeader === true && <div className='tbl-headings justify-content-between fixed-Header top-0' style={{ background: '#e9e9e9' }}>
                ...
                <span className="toolbox">
                    {items.taskProfile != true && items?.showCreationAllButton === true && <>
                        ...
                        {items?.protfolioProfileButton != true && items?.hideAddActivityBtn != true && <>{table?.getSelectedRowModel()?.flatRows.length === 1 || (table?.getSelectedRowModel()?.flatRows[0]?.original?.TaskType?.Title != "Task" || items?.MasterdataItem.Item_x0020_Type == "Sprint" || items?.MasterdataItem.Item_x0020_Type == "Project") ? <button type="button" className="btn btn-primary" title='Add Activity' style={{ backgroundColor: `${portfolioColor}`, borderColor: `${portfolioColor}`, color: '#fff' }} onClick={() => openCreationAllStructure("Add Activity-Task")}>Add Activity-Task</button> :
            ...
            {showPagination === true && table?.getFilteredRowModel()?.rows?.length > table.getState().pagination.pageSize ? <div className="d-flex gap-2 items-center mb-3 mx-2 pagnationpanel">
                <button
                    className="border "
                ...
                <span className="flex items-center gap-1 textpannel">
                    <div className='titleblock'>Page <strong>
