...
const Picker = (item: any) => {
    ...
    const loadSmartMetaDataItems = function () {
        ...
        var url = (`${siteUrls}/_api/web/lists/getbyid('136503cd-706e-4466-941f-eb2dcb39db7f')/items?$select=Id,Title,ParentID,TaxType&$top=4999&$filter=TaxType eq '` + SmartTaxonomyName + "'")
    ...
    const customHeader = () => {
        return (
            <>
                <div className="subheading">Select {item?.usedFor}</div>
                            ...
                            <div className="mb-2 col-sm-5 p-0">
                                <div className="position-relative">
                                    <div className="gap-2 justify-content-start valign-middle">
                                        ...
                                        <input type="text" className="form-control  searchbox_height mt-20 mb-3" value={value} onChange={onChange} placeholder={`Search ${item?.usedFor}`} />
                                        <span style={{ position: 'absolute', top: '44px', right: '10px' }}><BsSearch /></span>
                                        {/* <span className=""><input className="form-check-input me-1 rounded-0" style={{width: '15px', height: '15px'}} defaultChecked={isSearchWithDesciptions} onChange={() => setIsSearchWithDesciptions(isSearchWithDesciptions ? false : true)} type="checkbox" value="" /></span>
                                        <label className="small">Include description (info-icons) in search</label> */}
                                    </div>
                                    {/* <input type="checkbox" defaultChecked={isSearchWithDesciptions} onChange={() => setIsSearchWithDesciptions(isSearchWithDesciptions ? false : true)} className="form-check-input me-1 rounded-0" style={{ width: "15px", height: "15px" }} /> <label>Include description (info-icons) in search</label> */}
                                    {/* <input type="text" className="form-control searchbox_height mt-1" value={value} onChange={onChange} placeholder={`Search ${item?.usedFor}`} />
                                    ...
                                    {select.map((val: any) => {
                                        return (
                                            <span className="block me-1">
                                                <span>{val.Title}</span>
                                                <span className="bg-light ms-2 svg__icon--cross svg__iconbox" onClick={() => deleteSelectedCat(val)}></span>
                                            </span>
                                        )
                                    })}
                                </div> : null}
                                        ...
                                        <>
                                            <li onMouseEnter={() => HoverFirstLevel(item.Id)} onMouseLeave={HoverOutFirstLevel} key={item.Id}>
                                                {item.Item_x005F_x0020_Cover != null ?
                                                    <p onClick={() => selectPickerData(item)} className='mb-0' >
                                                            ...
                                                            <>
                                                                {child1.Title != null ?
                                                                    <li onMouseEnter={() => HoverSecondLevel(child1.Id)} onMouseLeave={HoverOutSecondLevel}>
                                                                        <p onClick={() => selectPickerData(child1)} className='mb-0'>
